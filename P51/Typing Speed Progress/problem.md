...
## Description
A typing tutor software logs Words Per Minute (WPM) for every test taken. For each test, the software shows a 'progress span', representing the number of consecutive previous tests (including the current one) where the WPM was less than or equal to the current score. Given the list of WPM scores, calculate this span.
