...
A hospital's patient records are stored in a sorted m x n matrix, where each cell is a patient ID. The sorting follows two rules:
    1. Each row is sorted by ID.
    2. The first ID in a row is greater than the last ID in the previous row.

Given a target patient ID, return 1 if the record is found or 0 otherwise. 
