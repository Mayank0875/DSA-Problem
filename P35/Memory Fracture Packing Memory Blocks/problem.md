...
Memory Fracture: Packing Memory Blocks
...
In a futuristic neural archive, fragments of memory are stored as identical rectangular blocks. Each block occupies **w** units in width and **h** units in height. To preserve data integrity, all **n** blocks must be placed flat inside a single square memory segment without overlapping, and rotation of blocks is prohibited. Your task is to determine the smallest possible side length of such a square segment that can contain all **n** blocks.
