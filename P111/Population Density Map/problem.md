...
## Description
A city planner has population counts for city blocks along a main road. To determine service needs, calculate the average population density centered on each block.

You are given a **0-indexed** array `nums` of `n` integers representing population counts and an integer `k`.
...
## Input Format
- The first line contains an integer `n`, representing the number of population counts.
