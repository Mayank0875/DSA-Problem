...
## Description
Users reacted to a post with various emojis.

The reactions are represented as a string $S$.
However, the order of the emojis is not fixed. The display bar shows these emojis in a linear sequence.

Your task is to calculate the **total number of distinct reaction bars** that can be formed using the characters of the string $S$. Since the number of possibilities can be very large, print the result modulo $10^9 + 7$.
