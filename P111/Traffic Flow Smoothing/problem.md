...
## Description
Traffic sensors report car counts per minute. To control traffic lights efficiently, the system calculates the average flow rate centered on each minute.

You are given a **0-indexed** array `nums` of `n` integers representing car counts and an integer `k`.
...
## Input Format
- The first line contains an integer `n`, representing the number of car counts.
