...
## Description
A solar panel array logs energy output every hour. To identify cloud cover impact, you need to find the energy output of the nearest future hour that is strictly lower than the current hour. Given the sequence of energy output values, find the next lower value for each reading. If output increases or stays constant in the future, output -1.
