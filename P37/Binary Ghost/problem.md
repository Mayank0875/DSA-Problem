...
A mischievous binary ghost wants to spread a secret bit‑pattern across the digital realm. The original pattern is held by the ghost, and it must create exactly n additional copies before dawn. There are two spectral duplicators available.  
The first duplicator needs x seconds to copy one pattern (from the original or any existing copy), and the second needs y seconds. The ghost can operate either duplicator or both at the same time.  
Help the binary ghost determine the minimum amount of time required to produce at least n new copies of the pattern, starting with just the original.
