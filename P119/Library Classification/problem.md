...
## Description
A librarian sorts categories. '{' is a category start, '}' is end.

The structure is represented by a **balanced** parentheses string $s$, where `'{'` signifies category and `'}'` signifies subcategory.

The librarian wants to index a specific topic that has a nesting depth of exactly $k$. They can choose to skip certain shelves of the structure, effectively using a **subsequence** of the original string. However, the chosen subsequence must still form a valid, balanced sequence.
