...
## Description
A game server records player command IDs. A known bug causes certain commands to be sent exactly twice, causing a 'double-cast' glitch. The developers need to identify which command IDs are affected by this specific bug from a session log. Return the IDs of commands that were logged exactly twice.
