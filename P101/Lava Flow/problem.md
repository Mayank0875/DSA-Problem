...
## Description
Lava cools in a sequence `s`. Gas bubbles burst, removing sections. How many unique rock formations can be created?

Your task is to determine the total number of distinct non-empty formations that can be formed. Two formations are considered distinct if they are not identical strings. As the number of possibilities can be substantial, print the answer modulo $10^9 + 7$.
