...
## Description
An ice core has layers `s`. Melting destroys some layers. Scientists calculate the number of unique climate histories preserved in the ice.

Your task is to determine the total number of distinct non-empty histories that can be formed. Two histories are considered distinct if they are not identical strings. As the number of possibilities can be substantial, print the answer modulo $10^9 + 7$.
