...
## Description
A factory produces items with serial numbers. A quality check compares the output sequence of two machines. Find the longest continuous run of serial numbers that both machines produced in the same order (indicating a sync issue). Calculate the run length.
