...
A memory champion stores items in a grid-like "memory palace" (`board`). Each room has a letter. To recall a `word`, they walk from room to adjacent room (up, down, left, or right). They cannot re-enter a room on the same recall path. Can they recall the given `word` from their palace?
## Examples
