...
A team of `n` volunteers is planting `t` trees for a reforestation project. Each volunteer `i` has a different planting speed, taking `k[i]` seconds to plant a single sapling. All volunteers can work in parallel. What is the minimum time needed for the team to plant at least `t` trees?
