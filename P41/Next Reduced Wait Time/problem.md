...
## Description
You are managing a theme park queue and have predicted wait times for every hour of the day. Visitors at the current hour want to know the wait time of the nearest future hour that is strictly shorter than the current wait. This helps distribute the crowd effectively. Given the sequence of hourly wait times, determine the next strictly smaller wait time for each hour. If the wait times only get longer or stay the same, output a specific indicator.
