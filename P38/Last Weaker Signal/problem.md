...
## Description
You are a telecommunications engineer analyzing signal strength data from a mobile tower. The data consists of signal strength measurements taken at regular intervals. To filter out noise and establish baseline connectivity, you need to find the signal strength of the nearest previous measurement that was strictly weaker than the current one. This helps in identifying local minima in signal quality. For every measurement in the sequence, your goal is to report the value of the closest preceding measurement that was strictly lower in strength. If all previous measurements were stronger or equal, or if it is the first measurement, you should signify this in the output. Your task is to analyze the entire signal log.
