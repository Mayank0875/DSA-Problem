...
## Description
A ski resort records the depth of fresh snow every morning. For each day, they calculate a 'snowfall span': the number of consecutive previous days (including the current one) where the snow depth was less than or equal to the current day. Given the snow depth logs, calculate this span.
