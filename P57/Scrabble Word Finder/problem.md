...
## Description
You are playing a word game. You have a rack of letter tiles and a dictionary of valid words. You want to calculate the total score you could get if you played every possible word from the dictionary that can be formed using your current tiles. The score of a word is equal to its length. Each tile can be used once per word, but the rack resets for each new word check.
