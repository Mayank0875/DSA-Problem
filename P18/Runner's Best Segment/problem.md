...
Runner's Best Segment

## Slug

runners-best-segment
...
A marathon runner's performance is split into `n` segments, with their points for each segment stored in a `Performance Log`. A "burst" is a contiguous block of `k` segments. Find the `maximum total points from any contiguous 'k'-segment burst`.
