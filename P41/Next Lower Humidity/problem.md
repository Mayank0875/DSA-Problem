...
## Description
A greenhouse control system logs humidity percentages. For each reading, the system calculates the humidity of the nearest future reading that is strictly lower to predict drying cycles. Given the sequence of humidity logs, find the next lower value for each entry. If humidity stays high or increases, indicate that no lower value follows.
