...
## Description
A user tracks steps taken during different intervals of a walk.

The user has a sequence of intervals lined up in a row. The $i$-th interval corresponds to `nums[i]` steps.

The user wants to track if the collection contains a valid "**Milestone Segment**." A Milestone Segment is defined as a continuous sub-segment of intervals that meets two conditions:
1. The sequence must contain **at least two** intervals.
2. The total sum of the steps in the sequence must be perfectly divisible by $k$ (the goal fraction).

Recall that 0 is always a multiple of $k$.

Your task is to determine if such a sequence exists. Return `true` if a Milestone Segment is found, and `false` otherwise.
