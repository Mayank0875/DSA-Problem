...
## Description
An astronomer adjusts the focus. '(' is a convex lens, ')' is concave.

The structure is represented by a **balanced** parentheses string $s$, where `'('` signifies magnifying and `')'` signifies reducing.

The astronomer wants to focus the image that has a nesting depth of exactly $k$. They can choose to skip certain lenses of the structure, effectively using a **subsequence** of the original string. However, the chosen subsequence must still form a valid, balanced sequence.
