...
## Description
A deli artist stacks a mega-sandwich. '[' is an ingredient layer, ']' is bread.

The structure is represented by a **balanced** parentheses string $s$, where `'['` signifies adding layer and `']'` signifies closing layer.

The artist wants to build a masterpiece that has a nesting depth of exactly $k$. They can choose to skip certain ingredients of the structure, effectively using a **subsequence** of the original string. However, the chosen subsequence must still form a valid, balanced sequence.
