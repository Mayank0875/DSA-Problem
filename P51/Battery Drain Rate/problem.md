...
## Description
A smartphone records the battery percentage drained every hour. For each hour, the system calculates a 'drain streak', which is the number of consecutive previous hours (including the current one) where the drain amount was less than or equal to the current hour. Given the drain logs, calculate this streak.
