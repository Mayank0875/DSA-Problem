...
## Description
A sensor drops into a volcano. '(' is a chamber, ')' is a vent.

The structure is represented by a **balanced** parentheses string $s$, where `'('` signifies chamber and `')'` signifies vent.

The sensor wants to record heat that has a nesting depth of exactly $k$. They can choose to skip certain pockets of the structure, effectively using a **subsequence** of the original string. However, the chosen subsequence must still form a valid, balanced sequence.
