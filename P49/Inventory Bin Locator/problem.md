...
## Description
A warehouse stores parts in bins labeled with capacity codes, sorted by size. A worker has a part of a specific size and needs to place it in a bin. They must find the first bin in the sorted row that has a capacity code greater than or equal to the part's size to ensure it fits properly.
