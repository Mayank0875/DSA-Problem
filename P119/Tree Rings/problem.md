...
## Description
A dendrologist studies tree growth rings. '(' is an inner ring start, ')' is the ring end.

The structure is represented by a **balanced** parentheses string $s$, where `'('` signifies moving inward and `')'` signifies moving outward.

The scientist wants to analyze a climate era that has a nesting depth of exactly $k$. They can choose to skip certain years of the structure, effectively using a **subsequence** of the original string. However, the chosen subsequence must still form a valid, balanced sequence.
