...
You are given a sorted array of distinct integers and a target value. Your task is to find the index where the target should be inserted to maintain the array's sorted order.

If the target value is already present, return its index. If not, return the index where it would be if it were inserted.
The index should point to the first element that is greater than or equal to the target.
If all elements are smaller, the target should be placed at the end.
