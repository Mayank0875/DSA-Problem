...
## Description
You are analyzing a user's daily activity. For each day's step count, you want to find the step count of the nearest future day where they were strictly less active (fewer steps). Given the list of daily step counts, find the next lower value for each day. If the user maintains or exceeds their activity level in all future days, report -1.
