...
## Description
A security system generates a salt string `s`. It derives keys by taking subsequences of the salt. How many unique keys can be generated?

Your task is to determine the total number of distinct non-empty keys that can be formed. Two keys are considered distinct if they are not identical strings. As the number of possibilities can be substantial, print the answer modulo $10^9 + 7$.
