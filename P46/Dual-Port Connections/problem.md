...
## Description
A network switch logs the device IDs connected to its ports. A healthy redundancy configuration requires devices to be connected to exactly two ports for failover. The network admin needs a list of all device IDs that satisfy this specific redundancy condition (connected exactly twice) to verify the setup.
