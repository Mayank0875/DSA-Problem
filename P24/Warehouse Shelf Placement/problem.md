...
You are given a sorted list of box weights on a warehouse shelf. A new box with `target` weight arrives and must be placed on the shelf while maintaining the sorted order.

Your task is to find the index where this new box should be placed.
The index should point to the first box that has a weight greater than or equal to the new box's weight.
If all existing boxes are lighter, it is placed at the end.
