...
An alchemist has a `board` (grid) of base elements (letters). They want to know if they can form a new compound (`word`) by combining adjacent elements. The combination process must follow a path (up, down, left, or right) and cannot use the same element (cell) twice for one compound.
## Examples
