...
## Description
Lighthouses cover a coastline loop. Some are active ('1'). Engineers can focus beams to illuminate sectors $d$ units away.
...
You want to transform the system so that all elements become lit. You have a special beam focus that allows you to perform an operation. In one operation, you can choose a shift amount $d$ ($1 \le d \le n$). This operation spreads the lit state cyclically to the right by $d$ positions. Specifically, if an element at index $i$ is lit, the element at index $(i+d) \% n$ also becomes lit (if it wasn't already).
