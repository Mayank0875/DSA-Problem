...
## Description
A factory produces metal balls which are sorted by weight. A quality control test requires a ball of at least a certain target weight. The robotic arm needs to find the first ball on the conveyor (which is sorted) that meets this weight requirement.
