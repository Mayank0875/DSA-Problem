...
## Description
You are analyzing heart rate data collected during a recovery phase after exercise. For each recorded heart rate beat-per-minute (BPM) value, you want to find the nearest future reading that is strictly lower. This helps visualize the speed of recovery. Given the sequence of BPM readings, find the next lower value for each entry. If the heart rate stabilizes or increases for the remainder of the session, indicate that no lower value follows.
