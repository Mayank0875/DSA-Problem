...
Imagine processing a stream of bytes, each with a certain value. As you examine each byte in the sequence, you look back (to the left) and want to know the value of the nearest previous byte that is strictly larger than the current one. If all earlier bytes are smaller or equal, or if it is the first byte, there is no such larger byte to the left. Your task is to determine this larger byte's value for every position in the stream.
