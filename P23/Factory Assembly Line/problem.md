...
A factory has `n` parallel assembly lines. The factory has an order for `t` products. Each assembly line `i` runs at a different speed, taking `k[i]` seconds to produce one product. All lines can run at the same time. Find the minimum time required to produce at least `t` products.
