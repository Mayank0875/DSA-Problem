...
A 1D cross-section of a glacier is represented by an array `arr` of `n` elevation points. The glacier rises to a central peak and then falls off.
    1. n >= 3
    2. `elev[0] < elev[1] < ... < elev[i-1] < elev[i]`
    3. `elev[i] > elev[i+1] > ... > elev[n-1]`

Find the index of the glacier's highest point.
