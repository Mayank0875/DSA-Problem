...
## Description
A chemist has a tray of elements identified by atomic number. They need to sort the tray so that rare elements (those present in small quantities) are accessed first. If quantities are equal, the element with the higher atomic number takes precedence.
