...
You are studying a sequence of spectral intensity measurements captured over time. For each measurement, you need to find the intensity value of the nearest previous measurement that is strictly lower than the current one. If no such previous measurement exists (either because it is the first measurement or all earlier intensities are higher or equal), you should indicate this. This analysis helps identify recent drops in intensity within the spectrum. Your task is to compute this value for every measurement given the intensity sequence.
