...
A goblin king wants to mint `t` new gold coins. He has `n` rickety minting machines. Each machine `i` is unreliable and slow, taking `k[i]` seconds to mint a single coin. All machines can be operated in parallel. What is the minimum time the goblins must work to mint at least `t` coins?
