...
## Description
A commuter starts at Station 1 and needs to get to Station n. The train lines are express and one-way towards the city center.

There are n stations numbered from 1 to n.
    1. Station 1 is the starting point.
    2. Station n is the destination.

Between some stations, there are one-way tracks that allow movement from u to v. The system contains no loops (directed acyclic graph), meaning you can only move forward towards the goal.
...
- The next m lines each contain two integers u and v, representing a one-way track from u to v.
