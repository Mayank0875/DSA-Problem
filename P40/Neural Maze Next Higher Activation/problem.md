...
Neural Maze: Next Higher Activation
...
In a simulated neural maze, each neuron fires with a certain activation level at successive time steps. For every time step, we need to find the activation level of the nearest future neuron that exhibits a strictly higher activation. This information helps researchers understand the propagation of stronger signals through the maze. If there is no later neuron with a higher activation (either because it is the last step or all subsequent activations are lower or equal), this should be indicated. Can you determine this for each time step?
