...
## Description
A time capsule contains items `s`. Water damage destroys some items. Future historians calculate how many unique collections could be recovered.

Your task is to determine the total number of distinct non-empty collections that can be formed. Two collections are considered distinct if they are not identical strings. As the number of possibilities can be substantial, print the answer modulo $10^9 + 7$.
