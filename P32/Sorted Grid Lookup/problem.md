...
You are given an m x n integer grid. This grid is special because it is sorted in a specific way:
    1. Each row is sorted from left to right.
    2. The entire grid is sorted as if it were a single, flattened array.

Given an integer target, return 1 if target is in the grid or 0 otherwise. 
