...
## Description
You are analyzing traffic flow data on a highway recorded in consecutive time intervals. For each interval, you want to identify the speed of the nearest future interval where the traffic moves strictly slower than the current speed. This helps in predicting congestion build-up. Given a sequence of average speeds, your task is to find the next strictly lower speed reading for each interval. If the traffic speed never drops below the current level in the future, output -1.
