...
A sensor takes `n` temperature readings in a cross-section of a magma chamber. The temperature is coolest at the edges and hottest at a single point in the middle, forming a "mountain array":
    1. n >= 3
    2. `temp[0] < temp[1] < ... < temp[i-1] < temp[i]`
    3. `temp[i] > temp[i+1] > ... > temp[n-1]`

Find the index of the chamber's core (the peak temperature).
