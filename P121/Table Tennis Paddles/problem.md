...
## Description
A rec center sets up ping pong tables. Each table ID needs exactly two paddles.

The crate has a list of paddles, represented by their IDs. For the game prep, every type of paddle in the list must appear **exactly twice**.

You can perform two types of operations:
1. **Add**: Add a new paddle of an existing ID (cost: 1).
2. **Remove**: Remove an existing paddle from the list (cost: 1).
