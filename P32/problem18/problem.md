...
An employee directory is stored as an m x n matrix of employee IDs. The directory is sorted to have these properties:
    1. Each row (department) is sorted by employee ID.
    2. The first ID in each department is greater than the last ID in the previous department.

Given a target employee ID, return 1 if the employee is in the directory or 0 otherwise. 
