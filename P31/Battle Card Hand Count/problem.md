...
You have a collection of $N$ battle cards, with attack values $candidates_i$. Your collection may contain multiple copies of the same card. You need to build a "hand" for your next turn with a total attack value of exactly $T$. Your task is to find the *total count* of unique combinations of cards from your collection that sum to $T$. Each card may be used *only once*.
