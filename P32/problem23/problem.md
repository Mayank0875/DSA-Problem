...
You are analyzing sensor data from an m x n grid. The data was recorded in a way that guarantees two properties:
    1. Each row of sensor readings is sorted.
    2. The first reading in any row is greater than the last reading in the previous row.

Given a target sensor reading, return 1 if that reading exists in the grid or 0 otherwise. 
