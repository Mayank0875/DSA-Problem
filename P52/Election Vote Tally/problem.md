...
## Description
In a unique voting system, candidates (represented by ID) who received the fewest votes are eliminated first. Sort the candidates based on their vote count in ascending order. If candidates tie in votes, the one with the higher ID is processed first.
