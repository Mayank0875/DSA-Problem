...
A 1D array represents a cross-section of a pyramid. The height values in the array strictly increase to the center (the peak) and then strictly decrease.
    1. n >= 3
    2. `h[0] < h[1] < ... < h[i-1] < h[i]`
    3. `h[i] > h[i+1] > ... > h[n-1]`

Find the index of the pyramid's highest point.
