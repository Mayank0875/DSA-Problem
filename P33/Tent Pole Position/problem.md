...
The height profile of a tent is given by an array `arr` of `n` measurements. The tent canvas rises from the ground to a central pole (the peak) and then falls back to the ground.
    1. n >= 3
    2. `h[0] < h[1] < ... < h[i-1] < h[i]`
    3. `h[i] > h[i+1] > ... > h[n-1]`

Find the index of the central pole (the peak height).
