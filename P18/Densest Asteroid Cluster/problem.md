...
A space probe scans `n` adjacent sectors of an asteroid field. The `Scan Data` (an array) shows the number of asteroids in each sector. A "cluster" is any `k` contiguous sectors. Find the `maximum total number of asteroids in any contiguous 'k'-sector cluster`.
