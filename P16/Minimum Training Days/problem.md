...
An athlete logs their daily running distance in an array `nums`. Given this list of positive distances and a positive `target` mileage, find the length of the shortest contiguous training block (subarray) where the total distance run is greater than or equal to the `target`. If they never meet the target, return 0.
