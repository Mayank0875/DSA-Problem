...
A grand library has tasked its `n` scribes with copying `t` ancient scrolls. Each scribe `i` works at a different speed, taking `k[i]` seconds to copy one scroll. All scribes can work in parallel. What is the minimum time required for them to finish copying all `t` scrolls?
