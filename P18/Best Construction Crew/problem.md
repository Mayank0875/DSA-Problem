...
A construction manager has `n` workers in a line. The `Efficiency List` (an array) shows the efficiency rating of each worker. A "crew" consists of `k` adjacent workers. Find the `maximum total efficiency of any contiguous 'k'-worker crew`.
