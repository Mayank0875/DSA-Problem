...
## Description
A game engine logs Frames Per Second. To display a readable counter, it shows the average FPS over the last few seconds.

You are given a **0-indexed** array `nums` of `n` integers representing FPS values and an integer `k`.
...
## Input Format
- The first line contains an integer `n`, representing the number of FPS values.
