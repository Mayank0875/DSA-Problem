...
## Description
A caver explores a deep vertical cave system. The cave map is a string where '(' represents a drop and ')' represents a climb out.

The structure is represented by a **balanced** parentheses string $s$, where `'('` signifies descending and `')'` signifies ascending.

The explorer wants to find a route that has a nesting depth of exactly $k$. They can choose to skip certain sections of the structure, effectively using a **subsequence** of the original string. However, the chosen subsequence must still form a valid, balanced sequence.
