...
## Description
An audio engineer cleans up reverb. '{' is an echo start, '}' is the decay.

The structure is represented by a **balanced** parentheses string $s$, where `'{'` signifies echo start and `'}'` signifies echo end.

The engineer wants to filter a frequency that has a nesting depth of exactly $k$. They can choose to skip certain waves of the structure, effectively using a **subsequence** of the original string. However, the chosen subsequence must still form a valid, balanced sequence.
