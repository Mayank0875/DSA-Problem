...
## Description
A park op counts guests arriving in minute intervals.

The operator has a sequence of intervals lined up in a row. The $i$-th interval corresponds to `nums[i]` guests.

The operator wants to manage if the collection contains a valid "**Full Ride**." A Full Ride is defined as a continuous sub-segment of intervals that meets two conditions:
1. The sequence must contain **at least two** intervals.
2. The total sum of the guests in the sequence must be perfectly divisible by $k$ (the ride capacity).

Recall that 0 is always a multiple of $k$.

Your task is to determine if such a sequence exists. Return `true` if a Full Ride is found, and `false` otherwise.
