...
Previous Lower Signal Strength
...
You're analyzing a sequence of silent transmission signal strengths recorded over time. For each transmission, you want to find the strength of the nearest previous transmission that was strictly lower than the current one. If no such previous transmission exists (either because it's the first transmission or all earlier strengths were higher or equal), you should indicate this. This analysis helps understand recent drops in signal quality. Your task is to compute this value for every transmission given the recorded signal strengths.
