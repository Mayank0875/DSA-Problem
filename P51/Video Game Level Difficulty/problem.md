...
## Description
A game designer wants to analyze the difficulty curve of a new campaign. For each level, they want to determine the 'easier-streak': the number of consecutive levels immediately preceding and including the current one that had a difficulty rating less than or equal to the current level. Given the sequence of difficulty ratings, compute this value for each level.
