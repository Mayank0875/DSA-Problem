...
A project has `n` tasks, with estimated hours in `nums`. Given a `target` number of hours, find the length of the shortest contiguous sequence of tasks (subarray) that will take at least `target` hours to complete. All task hours are positive. If no such sequence exists, return 0.
