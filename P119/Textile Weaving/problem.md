...
## Description
A weaver controls the loom. '(' is a warp over, ')' is a warp under.

The structure is represented by a **balanced** parentheses string $s$, where `'('` signifies thread over and `')'` signifies thread under.

The artisan wants to create a pattern that has a nesting depth of exactly $k$. They can choose to skip certain threads of the structure, effectively using a **subsequence** of the original string. However, the chosen subsequence must still form a valid, balanced sequence.
