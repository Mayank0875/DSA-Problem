...
## Description
You are an energy analyst monitoring the hourly electricity usage of a factory. You have a dataset of consumption values in kilowatts. To identify surges in power demand, you need to find the usage value of the nearest previous hour that was strictly lower than the current hour's usage. This helps in pinpointing when the current high load began to ramp up. For each hourly reading, you must find the value of the most recent preceding reading that was strictly smaller. If no such reading exists because usage has been high or constant, you should report a specific indicator. Your task is to process the power usage log.
