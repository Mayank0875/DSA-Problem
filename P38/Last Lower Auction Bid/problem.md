...
## Description
You are analyzing the bidding history of a high-stakes auction. Bids are placed sequentially, and you have a list of all bid amounts. To understand the bidding momentum, you want to identify the amount of the nearest previous bid that was strictly lower than the current bid. Note that in this specific auction format, bids don't always strictly increase. For each bid in the sequence, you must find the value of the most recent preceding bid that was strictly smaller than the current one. If no such bid exists (e.g., it's the opening bid or all previous bids were higher), you must indicate this. Your task is to process the full list of bids.
