...
## Description
A logistics robot packs boxes inside boxes. '[' is a box opening, ']' is a box closing.

The structure is represented by a **balanced** parentheses string $s$, where `'['` signifies opening a box and `']'` signifies closing a box.

The robot wants to create a shipment that has a nesting depth of exactly $k$. They can choose to skip certain boxes of the structure, effectively using a **subsequence** of the original string. However, the chosen subsequence must still form a valid, balanced sequence.
