...
## Description
A building logs elevator trips per hour. A smoothed average helps optimize the elevator's resting floor logic.

You are given a **0-indexed** array `nums` of `n` integers representing trip counts and an integer `k`.
...
## Input Format
- The first line contains an integer `n`, representing the number of trip counts.
