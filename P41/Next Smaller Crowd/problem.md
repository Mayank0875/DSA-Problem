...
## Description
You are analyzing hourly footfall in a shopping mall. For each hour's count, you want to predict the count of the nearest future hour that will have strictly fewer people. This helps in scheduling cleaning breaks during quieter times. Given the sequence of visitor counts, find the next lower count for each hour. If the crowd size never decreases below the current level, indicate this.
