...
You have a gift card with an exact balance of `sum`. You are at a store with `n` items, each with a different price. You want to know if it's possible to buy a collection of items (a subset) that spends your *entire* gift card balance, with nothing left over. You can only buy each item once.
## Examples
