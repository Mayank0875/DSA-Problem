...
You are an alchemist trying to brew a specific potion. You have `n` ingredients, each with a "magic essence" value. The recipe states the potion requires a *total* essence of exactly `sum`. Any more or less, and the potion will explode! You need to know if it's possible to combine a subset of your available ingredients to reach this exact `sum`. Each ingredient can only be used once.
## Examples
