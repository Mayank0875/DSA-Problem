...
Hiker's Perfect Meal Plan

## Slug

hikers-perfect-meal-plan
...
A hiker is packing for a trip and has `n` different food items. Each item has a specific calorie count. To optimize their pack, they want to know if it's possible to select a group of items that add up to an *exact* calorie goal of `sum` for one day's meal. You must find out if a subset of the food items can sum to `sum`. Each item can be packed only once.
## Examples
