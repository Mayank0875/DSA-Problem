...
You are a programmer loading an embedded device with a very specific memory limit. You have `n` code modules, each with a known memory footprint (in KB). The device requires *exactly* `sum` KB of modules to be loaded to boot. Can you select a subset of your `n` modules that perfectly fills this memory requirement?
## Examples
