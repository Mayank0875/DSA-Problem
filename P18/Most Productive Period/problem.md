...
An employee's `Task Log` shows the number of tasks completed each day for `n` days. A "work period" is defined as `k` contiguous days. Find the `maximum total tasks completed in any contiguous 'k'-day period`.
