...
You are trying to download `t` identical files from a server. You have `n` parallel download connections. However, due to network conditions, each connection `i` has a different speed, taking `k[i]` seconds to download one file. All connections can operate simultaneously. What is the minimum time required to download all `t` files?
