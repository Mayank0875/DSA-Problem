## Title
Video Frame Compression

## Slug
video-frame-compression

## Difficulty
Easy

## Description
A video codec organizes data into Frames `()`, Slices `[]`, and Macroblocks `{}`. To decode the stream correctly, the data blocks must be perfectly nested. A corrupted stream might have a Slice ending before its internal Macroblocks are finished. Given the stream structure string, determine if the video file is valid or corrupted.

## Examples

### 1

#### Input
()[]{}

#### Output
Yes

#### Explanation
All opening brackets have matching closing brackets in the correct order.

### 2

#### Input
([)]

#### Output
No

#### Explanation
The bracket '[' is closed by ')' which is a mismatch in type and nesting order.

## Input Format
- A single line containing a string s consisting only of the characters '(', ')', '{', '}', '[' and ']'.

## Output Format
- Return true (printed as Yes) if the string is valid, and false (printed as No) otherwise.

## Constraints
- 1 ≤ length(s) ≤ 1e5
- s contains only parentheses '()', square brackets '[]', and curly braces '{}'.

## Time Limit
2 second

## Memory Limit
256 MB

## Tags
stack, string
