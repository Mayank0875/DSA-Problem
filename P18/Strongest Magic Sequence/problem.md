...
A wizard has `n` magic runes in a row, with their power levels in a `Rune Array`. A "spell" is formed by channeling `k` contiguous runes. Find the `maximum total power (sum) of any contiguous 'k'-rune spell`.
