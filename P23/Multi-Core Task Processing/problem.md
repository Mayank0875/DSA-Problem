...
A CPU has `n` cores. You need to run `t` identical, independent tasks. Each core `i` has different performance characteristics and takes `k[i]` seconds to complete one task. All cores can work in parallel. What is the minimum time required to complete all `t` tasks?
