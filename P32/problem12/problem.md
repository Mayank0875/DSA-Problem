...
You are looking for a specific seat in a concert hall. The hall's seating chart is an m x n matrix of seat numbers with these properties:
    1. Each row is sorted by seat number.
    2. The first seat number in any row is greater than the last seat number in the previous row.
