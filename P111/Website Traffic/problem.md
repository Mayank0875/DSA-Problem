...
## Description
Webmasters analyze hits per hour. A smoothed average helps identify peak traffic periods without being distracted by momentary spikes.

You are given a **0-indexed** array `nums` of `n` integers representing hit counts and an integer `k`.
...
## Input Format
- The first line contains an integer `n`, representing the number of hit counts.
