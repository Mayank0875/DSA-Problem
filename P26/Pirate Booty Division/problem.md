...
A pirate crew has `n` treasure chests, each with a different gold value. The captain wants to take a portion of the treasure that is worth *exactly* `sum` gold. You are the quartermaster, and you must determine if it is possible to pick a subset of the `n` chests to satisfy the captain's demand. Each chest can either be taken or left.
## Examples
