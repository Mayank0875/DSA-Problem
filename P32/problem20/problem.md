...
A gene database is stored as an m x n matrix of gene IDs. The database is highly optimized and has the following properties:
    1. Each row is sorted by gene ID.
    2. The first ID of each row is greater than the last ID of the previous row.

Given a target gene ID, return 1 if it is in the database or 0 otherwise. 
