## Title
Previous Lower Character Level
...
## Description
You are analyzing the progression of an RPG character. You have a list of the character's level recorded at the end of every game session. Due to a unique game mechanic, levels can fluctuate. You want to find the level recorded in the nearest previous session that was strictly lower than the current level. This helps in identifying recovery points after level drains. For each recorded level, your goal is to find the value of the most recent prior session where the level was strictly lower. If no such session exists, you must indicate this. Your task is to process the character level history.
