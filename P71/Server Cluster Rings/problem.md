...
## Description
Servers are clustered in redundancy rings. Each server links to two others.

There are n servers.
Rings have at least 2 servers.
Logs show one active link per server.
...
5 <-> 6.
Maximum:
These could be 3 separate rings of 2 servers each.
Minimum:
Since we only know one neighbor per server, these pairs could actually be segments of a bigger ring. All servers can be arranged into one single ring:
1 - 2 - 3 - 4 - 5 - 6 - 1.
...
- The second line contains n space-separated integers a1, a2, ..., an, where ai is the neighbor remembered by the i-th server.
