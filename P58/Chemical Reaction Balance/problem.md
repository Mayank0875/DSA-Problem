...
## Description
A chemist has several vials of solution, each with a specific pH level (represented as an integer). To create a stable compound, two vials must be mixed such that their total pH equals a target stability constant. Count the number of valid pairs of vials that can be mixed to achieve this exact stability.
