...
## Description
A DevOps engineer tracks server load percentage every hour. For each hour, they calculate a 'load span', defined as the number of consecutive previous hours (including the current one) where the load was less than or equal to the current hour's load. Given the sequence of load percentages, compute the load span for each hour.
