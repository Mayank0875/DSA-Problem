...
You are tracking N stocks, and their daily performance is simplified into an array A (0 for 'Loss', 1 for 'Gain'). You receive Q queries from your investors. Each query (L, R, Val) asks for the count of stocks in the range [L, R] that had the performance `Val` (0 or 1). Your final report must be the XOR sum of all these counts.
