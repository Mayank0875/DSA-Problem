...
You are given an array of positive integers `nums` representing daily donations for a charity, and a positive integer `target` representing the fundraising goal. Find the length of the shortest contiguous period of days (subarray) whose total donations are greater than or equal to `target`. If the goal is never met, return 0.
