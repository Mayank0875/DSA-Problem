...
You are a spaceship captain navigating a galactic sector map, which is an m x n grid of sector IDs. The map is sorted as follows:
    1. Each row is sorted by sector ID.
    2. The first ID in each row is greater than the last ID in the previous row.

Given a target sector ID, return 1 if it is on the map or 0 otherwise. 
