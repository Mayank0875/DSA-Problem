...
## Description
A travel aggregator tracks hotel room rates for a specific night over a period of time. For every rate quoted, they want to display the rate of the next upcoming quote that is strictly cheaper. This helps users decide if they should book now or wait. Given the list of quoted rates over time, find the next lower rate for each quote. If rates never drop below the current quote, indicate this.
