...
A quilt is made of lettered patches in a `board` (grid). A customer wants a specific pattern (`word`). Can you find this `word` by tracing adjacent patches (up, down, left, or right)? You cannot use the same patch twice for the same word. Your task is to check if the `word` exists.
## Examples
