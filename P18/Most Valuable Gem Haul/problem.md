...
A miner has found `n` gems in a contiguous vein. The `Gem Vein` is an array where each number is the value of a gem. The miner's bag can only hold `k` contiguous gems. What is the `maximum total value of any contiguous 'k' gems` the miner can collect?
