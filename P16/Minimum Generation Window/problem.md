...
A power grid's output is logged in an array `nums` (positive megawatts per minute). To prevent a blackout, the grid must generate at least `target` megawatts. Find the length of the shortest contiguous time window (subarray) whose total output is greater than or equal to `target`. If this is impossible, return 0.
