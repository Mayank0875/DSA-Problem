...
An archery squad of `n` archers is training for a competition. They must shoot a total of `t` arrows. Each archer `i` has a different firing rate, taking `k[i]` seconds to shoot one arrow. All archers can shoot at the same time. What is the minimum time required for the squad to shoot at least `t` arrows?
