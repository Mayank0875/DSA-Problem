...
You are examining a stream of bytes transmitted over a network. For each byte, you need to determine the value of the nearest previous byte that is strictly smaller than the current one. If there is no such previous byte (either because it is the first byte or all earlier bytes are greater than or equal to it), you should indicate this. This information helps uncover hidden patterns in the byte sequence. Your task is to compute this value for every position given the sequence of bytes.
