...
You are a security analyst monitoring a server's access log. The log is a sequence of `n` user IDs, known as the `Access Sequence`. An "anomaly" is a security risk where the same user ID appears at least twice `within a distance of 'k' log entries` from each other. Your task is to check the sequence and determine if such an anomaly exists.
