...
## Description
An audio engineer smooths out frequency spikes in a sound wave. Calculate the average amplitude for each time slice using its neighbors.

You are given a **0-indexed** array `nums` of `n` integers representing amplitudes and an integer `k`.
...
## Input Format
- The first line contains an integer `n`, representing the number of amplitudes.
