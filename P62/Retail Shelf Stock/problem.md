...
## Description
A store manager clears k items from a shelf to make room for a display. Items are identified by brand ID. The manager wants to minimize the number of different brands left on that specific shelf. What is the minimum unique brand count?
