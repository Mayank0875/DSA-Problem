...
## Description
Hydrologists measure the depth of a river at regular intervals. For each measurement, they calculate a 'shallowness span': the number of consecutive previous measurements (including the current one) that were less deep or equal to the current depth. Given the sequence of depth readings, calculate this span for every measurement point.
