...
## Description
A superintelligence builds computing shells around a star. '(' is a new shell, ')' is the exterior.

The structure is represented by a **balanced** parentheses string $s$, where `'('` signifies building shell and `')'` signifies sealing shell.

The AI wants to maximize processing that has a nesting depth of exactly $k$. They can choose to skip certain megastructures of the structure, effectively using a **subsequence** of the original string. However, the chosen subsequence must still form a valid, balanced sequence.
