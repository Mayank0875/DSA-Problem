...
The ancient vault can only be opened with an obsidian key. The master artisan possesses a single original key and must forge exactly **n** additional keys before the sunrise. There are two enchanted forges at his disposal.
The first forge can create a key in **x** seconds (using the original or any already forged key), and the second forge works in **y** seconds. The artisan may operate either forge alone or both simultaneously.  
Determine the minimum amount of time required to produce at least **n** new obsidian keys, starting with just the original key.
