...
A shipyard has `n` construction bays, each working on building ship components. The shipyard has a quota to produce `t` components. Each bay `i` has its own crew and tools, taking `k[i]` seconds to finish one component. All bays work in parallel. What is the minimum time needed to meet the quota of `t` components?
