...
## Description
A warehouse manager monitors the daily stock count of a popular item. For each day, they want to find the number of consecutive days ending on that day where the inventory level was less than or equal to the current level. This helps in understanding restocking cycles. Given the daily inventory counts, calculate this value for each day.
