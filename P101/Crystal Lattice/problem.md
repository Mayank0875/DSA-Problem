...
## Description
A crystal structure is defined by atoms `s`. Impurities cause atoms to be missing. How many distinct defective crystal structures can exist?

Your task is to determine the total number of distinct non-empty structures that can be formed. Two structures are considered distinct if they are not identical strings. As the number of possibilities can be substantial, print the answer modulo $10^9 + 7$.
