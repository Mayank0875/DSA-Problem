...
A coin collector has `n` rare coins, each with a different value. They want to create a display set for a competition. The competition rules state the total value of the set must be *exactly* `sum`. You must determine if it is possible for the collector to choose a subset of their coins to meet this exact value. Each coin can only be included once.
## Examples
