...
## Description
A video player monitors rendering times. To determine playback smoothness, it calculates the average frame time over a sliding window.

You are given a **0-indexed** array `nums` of `n` integers representing frame times and an integer `k`.
...
## Input Format
- The first line contains an integer `n`, representing the number of frame times.
