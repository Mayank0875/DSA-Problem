...
## Description
An astronomer is observing a stream of asteroids passing Earth. For each asteroid observed, they want to calculate the size of the nearest following asteroid that is strictly smaller. This helps in classifying debris fields. Given the diameters of the asteroids in sequence, find the diameter of the next smaller asteroid for each one. If all subsequent asteroids are larger or equal, output -1.
