...
## Description
An archaeologist places a stone $y$ on a pedestal $x$. The rune activates if the geometric difference (XOR) divides the value of the stone or pedestal.

The archaeologist defines a specific relationship between a fixed integer $x$ and a candidate integer $y$. A value $y$ is considered "**active**" with $x$ if:
...
Given the fixed value $x$ and the upper bound $m$, your task is to count the total number of active integers $y$.
...
## Output Format
- Return a single integer representing the count of active values.
