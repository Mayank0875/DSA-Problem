...
## Description
A YouTuber looks at the view counts of their uploaded videos in chronological order. For each video, they calculate a 'view streak': the number of consecutive previous videos (including the current one) that had fewer or equal views. Given the sequence of view counts, determine the streak for each video.
