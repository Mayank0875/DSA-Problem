...
A mysterious recursive routine is traversing a sequence of numbers, trying to glimpse the next “phantom” value that is strictly larger than the current one and appears later in the call chain. For each position in the sequence, determine the value of the nearest future element that the recursion would encounter which is greater than the current element. If there is no such later element (either because it is the last position or all subsequent values are lower or equal), denote this with -1. Compute this for every position in the array.
