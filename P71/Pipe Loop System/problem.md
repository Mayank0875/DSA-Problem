...
## Description
Pipes connect junctions in cooling loops.

There are n junctions.
Loops contain at least 2 junctions.
Each junction has one known pipe connection.
...
5 <-> 6.
Maximum:
These could be 3 separate loops of 2 junctions each.
Minimum:
Since we only know one neighbor per junction, these pairs could actually be segments of a bigger ring. All junctions can be arranged into one single ring:
1 - 2 - 3 - 4 - 5 - 6 - 1.
...
- The second line contains n space-separated integers a1, a2, ..., an, where ai is the neighbor remembered by the i-th junction.
