...
Imagine you are a cryptographer deciphering a long sequence of encoded symbols, each represented by an integer value. As you examine each symbol in the sequence, you look back at the previously decoded symbols (to your left) and want to know the value of the nearest earlier symbol that is strictly larger than the current one. If all earlier symbols are smaller or equal, or if you are at the first symbol, there is no such larger preceding symbol. Your task is to determine this larger symbol's value for every position in the sequence.
