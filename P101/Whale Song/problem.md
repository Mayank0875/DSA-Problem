...
## Description
A whale sings a complex song `s`. The sound distorts over distance, losing frequencies. How many unique song fragments can another whale hear?

Your task is to determine the total number of distinct non-empty fragments that can be formed. Two fragments are considered distinct if they are not identical strings. As the number of possibilities can be substantial, print the answer modulo $10^9 + 7$.
