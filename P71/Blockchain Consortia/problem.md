...
## Description
Nodes validate transactions in consortium rings.

There are n nodes.
Rings contain at least 2 nodes.
Logs show one peer connection per node.
...
5 <-> 6.
Maximum:
These could be 3 separate rings of 2 nodes each.
Minimum:
Since we only know one neighbor per node, these pairs could actually be segments of a bigger ring. All nodes can be arranged into one single ring:
1 - 2 - 3 - 4 - 5 - 6 - 1.
...
- The second line contains n space-separated integers a1, a2, ..., an, where ai is the neighbor remembered by the i-th node.
