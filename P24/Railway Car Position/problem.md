...
A train consists of cars sorted by their serial number. A new car with a `target` serial number needs to be added to the train.

Your task is to determine the index where this new car should be inserted to keep the train sorted.
The index should point to the first car that has a serial number greater than or equal to the new car's number.
If all cars have smaller numbers, it's attached to the end.
