...
## Description
A warehouse barcode scanner sometimes malfunctions and scans items multiple times. The inventory manager wants to identify specific items that were accidentally scanned exactly twice, as this indicates a specific type of system glitch. Given the log of scanned item IDs, list the IDs that appear exactly twice so the manager can investigate.
