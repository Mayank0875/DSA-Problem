...
## Description
A secure server stores encrypted files. A decryption algorithm only works on files with an even file size (in bytes). As files are appended or truncated, report the total size of all files that are currently compatible with the decryption algorithm.
