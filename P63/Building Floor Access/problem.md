...
## Description
An elevator log records $n$ stops. The building has 5 floors. A specific maintenance mode requires the elevator to only move upwards or stay on the same floor (non-decreasing floor numbers). Calculate the number of valid trip logs of length $n$.
