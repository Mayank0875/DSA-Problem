...
## Description
Wind shapes a dune profile `s`. Erosion removes peaks. How many unique dune silhouettes can be formed against the horizon?

Your task is to determine the total number of distinct non-empty silhouettes that can be formed. Two silhouettes are considered distinct if they are not identical strings. As the number of possibilities can be substantial, print the answer modulo $10^9 + 7$.
