...
In a covert operation, agents must transmit **n** identical rectangular data packets.  
Each packet occupies a rectangle that is **w** units wide and **h** units tall.  
To keep the transmission undetectable, all packets must be placed flat inside a single square transmission window.  
The packets cannot be rotated. Your task is to determine the smallest possible side length of the square window that can contain all **n** packets without any overlap.
