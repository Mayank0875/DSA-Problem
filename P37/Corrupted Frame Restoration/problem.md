...
A digital archive contains a single pristine frame, but a virus has corrupted many copies. The archivist must restore exactly n additional frames before the system reboot. There are two repair modules available.
The first module restores one frame (from the original or any already restored copy) in x seconds, and the second module restores one frame in y seconds. The archivist can use either module or both simultaneously.  
Help determine the minimum amount of time required to produce at least n new restored frames, starting with just the original.
