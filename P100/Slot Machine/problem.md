...
## Description
A slot machine reel stops on a sequence of symbols.

The symbols are represented as a string $S$.
However, the order of the symbols is not fixed. The payout is determined by the specific arrangement of symbols.

Your task is to calculate the **total number of distinct outcomes** that can be formed using the characters of the string $S$. Since the number of possibilities can be very large, print the result modulo $10^9 + 7$.
