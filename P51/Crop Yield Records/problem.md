...
## Description
A farmer records the crop yield (in tons) for consecutive harvest seasons. For each season, they want to calculate the 'yield span': the number of consecutive previous seasons (including the current one) where the yield was less than or equal to the current season. Given the yield data, calculate this span for each season.
