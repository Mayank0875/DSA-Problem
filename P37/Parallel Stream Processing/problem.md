...
A data center needs to replicate a critical data packet across its servers before the next batch job starts. The original packet is already available, and exactly **n** additional copies must be produced. There are two parallel data streams that can be used for replication.
The first stream takes **x** seconds to copy one packet (from the original or any existing copy), and the second stream takes **y** seconds. The streams can be used independently or simultaneously.
Determine the minimum amount of time required to produce at least **n** new copies of the packet, starting with just the original.
