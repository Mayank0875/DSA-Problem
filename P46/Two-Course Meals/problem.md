...
## Description
A restaurant tracks Table IDs. For a specific 'Date Night' special, a table must order exactly two courses to qualify for a discount. The system logs the Table ID for every course served. Identify which Table IDs appear exactly twice in the service log.
