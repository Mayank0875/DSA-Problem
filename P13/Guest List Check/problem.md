...
You are a security guard at an exclusive event. You have a sorted list of all approved guest IDs. A person arrives and gives you their `target` ID. You must efficiently check your list to see if their ID is present. Your function should take the sorted list of IDs and the `target` ID, and return a simple confirmation. You must write an algorithm with O(log n) runtime complexity.
