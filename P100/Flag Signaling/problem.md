...
## Description
A ship has a set of colored flags to hoist on a mast. The message depends on the vertical order of the flags.

The set of flags is represented as a string $S$.
However, the order of the flags is not fixed. Identical colored flags are indistinguishable, reducing the number of unique messages.
