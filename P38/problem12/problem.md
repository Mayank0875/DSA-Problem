...
In the haunted realm of binary signals, a mischievous ghost leaves traces of its presence in a sequence of integer values. For each position in the sequence, you need to uncover the value of the nearest previous position where the ghost's intensity was strictly lower than the current intensity. If there is no such previous position (either because it is the first element or all earlier intensities are higher or equal), you must indicate this with -1. Determining these previous lower intensities helps to map the ghost's movement through the binary world. Compute this value for every position given the sequence of intensities.
