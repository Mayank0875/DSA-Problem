...
An online store's product catalog is structured as an m x n matrix. The product IDs are sorted with these two properties:
    1. Each row (category) is sorted by product ID.
    2. The first product ID in a new category is greater than the last ID in the previous category.

Given a target product ID, return 1 if it exists in the catalog or 0 otherwise. 
