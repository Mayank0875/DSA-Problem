...
In the annual Potion Brewing Contest, `n` alchemists must brew a total of `t` healing potions. Each alchemist `i` is a bit different, taking `k[i]` seconds to brew a single potion. All `n` alchemists can brew simultaneously. What is the minimum time in which they can collectively finish brewing at least `t` potions?
