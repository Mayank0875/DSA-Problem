...
## Description
A server logs ping times. To alert admins of real issues, the monitoring tool smooths the latency graph to ignore singular lag spikes.

You are given a **0-indexed** array `nums` of `n` integers representing latency values and an integer `k`.
...
## Input Format
- The first line contains an integer `n`, representing the number of latency values.
