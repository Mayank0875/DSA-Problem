...
## Description
A drone flies through dense foliage. '(' enters a canopy layer, ')' exits.

The structure is represented by a **balanced** parentheses string $s$, where `'('` signifies entering and `')'` signifies exiting.

The drone pilot wants to scan the biodiversity that has a nesting depth of exactly $k$. They can choose to skip certain leaf layers of the structure, effectively using a **subsequence** of the original string. However, the chosen subsequence must still form a valid, balanced sequence.
