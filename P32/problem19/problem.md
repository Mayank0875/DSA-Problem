...
You are given a train schedule as an m x n matrix of departure times (represented as integers). The schedule is organized as follows:
    1. Each row (platform) has its departure times sorted.
    2. The first departure time on any platform is later than the last departure time on the previous platform.

Given a target departure time, return 1 if it is on the schedule or 0 otherwise. 
