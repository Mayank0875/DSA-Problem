...
You are a librarian searching for a book in a large library. The library's shelves are organized as an m x n matrix. The book IDs on these shelves have the following properties:
    1. Each shelf (row) is sorted by book ID from left to right.
    2. The first book ID of each shelf is greater than the last book ID of the previous shelf.

Given a target book ID, return 1 if the book is in the library or 0 otherwise. 
