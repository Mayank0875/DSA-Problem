...
## Description
A search engine tracks query volume. To identify trending topics, it looks for sustained high averages in query counts.

You are given a **0-indexed** array `nums` of `n` integers representing query counts and an integer `k`.
...
## Input Format
- The first line contains an integer `n`, representing the number of query counts.
