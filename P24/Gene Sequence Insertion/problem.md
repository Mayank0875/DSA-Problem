...
You are a biologist working with a sorted list of gene IDs. A new gene with a `target` ID has been discovered and needs to be inserted into this sorted database.

Your task is to determine the correct index for this new gene ID.
The index should be the first one containing an ID greater than or equal to the `target` ID.
If all existing IDs are smaller, the new gene should be placed at the end of the list.
