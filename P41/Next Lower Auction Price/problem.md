...
## Description
In a Dutch auction, prices start high and drop. However, sometimes new lots are introduced at higher prices. For every price announcement, you want to find the nearest future price that is strictly lower. Given the sequence of announced prices, find the next lower price for each one. If the prices only go up or stay flat for the rest of the auction, indicate no lower price.
