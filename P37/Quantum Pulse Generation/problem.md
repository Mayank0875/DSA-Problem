...
The research lab needs to generate a burst of quantum pulses for an experiment! The lead scientist has a single initial pulse and must create exactly n additional pulses before the observation window closes. Two quantum pulse generators are available.
The first generator emits a pulse every x seconds (using either the original pulse or any generated pulse), and the second generator emits a pulse every y seconds. The scientists can operate either generator or both simultaneously. 
Help the lead scientist determine the minimum amount of time required to produce at least n new quantum pulses, starting with just the original pulse.
