...
A city's power usage over `n` hours is given. The usage forms a "mountain array": it climbs during the day, hits a peak in the evening, and falls at night.
    1. n >= 3
    2. `usage[0] < usage[1] < ... < usage[i-1] < usage[i]`
    3. `usage[i] > usage[i+1] > ... > usage[n-1]`

Find the index (hour) of the city's peak energy consumption.
