...
## Description
You are debugging a complex circuit board represented as a grid of component codes (letters). You need to verify if a specific signal path (a sequence of components) exists physically on the board. The signal can only travel between adjacent components. Check if the specific sequence is present.
