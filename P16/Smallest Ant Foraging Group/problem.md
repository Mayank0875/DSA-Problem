...
An ant colony needs to carry `target` units of food. The ants are in a line, and their individual carrying capacities are in the array `nums`. Find the length of the shortest contiguous line of ants (subarray) whose total capacity is greater than or equal to `target`. If no group can carry it, return 0.
