...
## Description
A music player shuffles a specific set of songs.

The song list is represented as a string $S$.
However, the order of the songs is not fixed. Identical songs (remixes) are treated as the same type.

Your task is to calculate the **total number of distinct queues** that can be formed using the characters of the string $S$. Since the number of possibilities can be very large, print the result modulo $10^9 + 7$.
