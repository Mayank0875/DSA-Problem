...
You are logging temperature readings from a sensor. The readings are stored in a sorted list. A new reading with a `target` temperature has just arrived.

Your task is to find the correct index to insert this new reading to maintain the sorted order.
The index should be the first position in the list with a temperature greater than or equal to the `target`.
If all previous readings are cooler, it is placed at the end.
