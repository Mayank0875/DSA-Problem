...
## Description
A node receives blockchain blocks with height indices. It attempts to sync the longest chain of consecutive block heights.

Given an unsorted array of integers `heights` representing the block heights, return the length of the longest consecutive sequence of blocks that can be synced to the chain.
