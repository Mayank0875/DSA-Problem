...
## Description
You are monitoring pressure sensors in a pipeline. Data is logged every minute. For safety analysis, you need to identify the pressure reading of the nearest future minute that is strictly lower than the current reading. This helps in understanding pressure release cycles. Given the sequence of pressure values, find the next strictly lower reading for each time step. If pressure only builds up or holds steady, report -1.
