...
An alchemist must fill a vial with exactly $T$ milliliters of potion. She has several ladles of different, fixed sizes (e.g., 2, 3, 5 ml). She can use any ladle an unlimited number of times. Your task is to find the *total number* of unique combinations of ladle pours that will sum exactly to the target volume $T$.
