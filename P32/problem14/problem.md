...
You are searching a large file cabinet, represented as an m x n matrix. The files are numbered and organized with two properties:
    1. Each drawer (row) is sorted by file number.
    2. The first file in any drawer has a higher number than the last file in the drawer above it.

Given a target file number, return 1 if it is in the cabinet or 0 otherwise. 
