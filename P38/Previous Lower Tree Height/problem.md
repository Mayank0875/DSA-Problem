...
## Description
You are a forestry expert analyzing a transect line through a forest. You have measured the heights of trees in a straight line. To study canopy structure and light competition, you want to find the height of the nearest previous tree that is strictly shorter than the current tree. This helps in identifying gaps where the current tree dominates. For each tree height, your objective is to find the value of the closest preceding tree that is strictly smaller. If all trees before it are taller or equal, you must indicate this. Your task is to analyze the sequence of tree heights.
