...
A book bindery has `n` workers. They have a large order for `t` hand-bound books. Each worker `i` has their own set of tools and a different speed, taking `k[i]` seconds to bind one book. All workers can bind books at the same time. What is the shortest time for the workshop to complete at least `t` books?
