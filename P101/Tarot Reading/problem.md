...
## Description
A fortune teller draws cards `s`. They choose to interpret only a subset of them in order. How many distinct fortunes can be told from this draw?

Your task is to determine the total number of distinct non-empty fortunes that can be formed. Two fortunes are considered distinct if they are not identical strings. As the number of possibilities can be substantial, print the answer modulo $10^9 + 7$.
