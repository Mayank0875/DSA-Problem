...
Binary Ghost: Previous Higher Binary
...
Imagine wandering through a haunted corridor filled with binary ghosts, each possessing a distinct power level. As you stand before each ghost, you glance to your left (the previous ghosts) and want to know the power level of the nearest ghost that is strictly stronger than the one you are currently facing. If all previous ghosts are weaker or have equal power, or if it's the first ghost, there is no such stronger ghost to the left. Your goal is to determine this stronger ghost's power level for each position along the corridor.
