...
## Description
A fitness watch tracks calories burned per minute. For each minute's data, the user wants to see the value of the nearest future minute where they burned strictly fewer calories (rest period). Given the sequence of calorie burn values, find the next lower value for each minute. If the intensity stayed the same or increased, output -1.
