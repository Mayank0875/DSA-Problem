...
## Description
A conveyor belt scales packages in sequence. For optimization, the sorting algorithm needs to know, for each package, the weight of the nearest following package that is strictly lighter. This helps in efficient bin packing. Given the stream of package weights, determine the weight of the next lighter package for each item. If all following packages are heavier or equal, indicate no lighter package exists.
