...
## Description
A gamer enters simulations. '(' is login, ')' is logout.

The structure is represented by a **balanced** parentheses string $s$, where `'('` signifies login and `')'` signifies logout.

The user wants to reach a server depth that has a nesting depth of exactly $k$. They can choose to skip certain sessions of the structure, effectively using a **subsequence** of the original string. However, the chosen subsequence must still form a valid, balanced sequence.
