...
You are creating a music playlist that must be exactly $T$ seconds long. You have a library of songs with different lengths (e.g., 2, 3, 5 seconds). Your task is to find the *total number* of unique combinations of these songs that sum exactly to the target duration $T$. You can add any song to the playlist multiple times.
