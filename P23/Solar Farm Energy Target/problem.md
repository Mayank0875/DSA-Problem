...
A solar farm has `n` solar panels. The farm's goal is to produce `t` units of energy. Each panel `i` produces energy at a different rate, taking `k[i]` seconds to generate one unit. All panels work in parallel. What is the minimum time needed for the farm to generate at least `t` units of energy?
