...
A chronomancer must generate at least n temporal echoes of a core event before the time vortex seals. The master chronal device holds the original event and can be used to create echoes. There are two temporal generators available.  
The first generator creates one echo in x seconds (from the original event or any existing echo), and the second generator creates one echo in y seconds. The chronomancer may operate either generator or both at the same time.  
Determine the minimum amount of time required to produce at least n new echoes, starting with just the original event.
