...
A city's addresses are organized on a grid. The house numbers in this m x n grid follow two rules:
    1. Each street (row) has house numbers sorted in increasing order.
    2. The first house number on a street is greater than the last house number on the previous street.

Given a target house number, return 1 if it exists in the grid or 0 otherwise. 
