...
In a cutting‑edge laboratory, researchers are arranging n identical quantum pulse modules for an experiment.  
Each module occupies a rectangle that is w units wide and h units long.  
All modules must be placed flat inside a single square containment field, and they cannot be rotated.  

Your task is to determine the smallest possible side length of the square field that can accommodate all n modules without any overlap.
