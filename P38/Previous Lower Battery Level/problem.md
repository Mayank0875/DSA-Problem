...
## Description
You are testing a new battery charging algorithm and have logged the battery percentage at fixed intervals during a complex charge-discharge cycle. To analyze charging efficiency and dips, you need to find the battery percentage of the nearest previous interval that was strictly lower than the current percentage. This helps identify moments of recovery after heavy usage. For each recorded battery percentage, you must find the value of the closest preceding recording that was strictly smaller. If all previous recordings were higher or equal, or if it is the first recording, you should indicate this with a specific value. Your task is to process the entire battery log.
