...
## Description
Prisoners walk in circles during exercise time.

There are n prisoners.
Circles have at least 2 prisoners.
Each prisoner follows one other.
...
5 <-> 6.
Maximum:
These could be 3 separate circles of 2 prisoners each.
Minimum:
Since we only know one neighbor per prisoner, these pairs could actually be segments of a bigger ring. All prisoners can be arranged into one single ring:
1 - 2 - 3 - 4 - 5 - 6 - 1.
...
- The second line contains n space-separated integers a1, a2, ..., an, where ai is the neighbor remembered by the i-th prisoner.
