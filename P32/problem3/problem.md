...
You are an astronomer searching a star chart, which is an m x n grid of star catalog numbers. The chart is organized with these properties:
    1. Each row is sorted by star number.
    2. The first star number in a new row is always larger than the last star number in the preceding row.

Given a target star number, return 1 if it is on the chart or 0 otherwise. 
