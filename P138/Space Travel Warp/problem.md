...
## Description
A ship at sector $x$ calculates jumps to sector $y$. A jump is stable if the warp metric (XOR) divides the sector number.

The pilot defines a specific relationship between a fixed integer $x$ and a candidate integer $y$. A value $y$ is considered "**stable**" with $x$ if:
