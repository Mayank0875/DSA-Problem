...
A wizard weaves a spell (`word`) by selecting runes from a `board` (grid). To cast the spell, they must select the runes in order. They can only move from one rune to an adjacent (up, down, left, or right) rune. They cannot use the same rune (cell) twice. Does the grid allow the casting of this `word`?
## Examples
