...
## Description
An artist tracks the daily stream count of their new single. For each day, they want to calculate a 'popularity span', defined as the number of consecutive previous days (including the current one) where the stream count was less than or equal to the current day. Given the daily stream numbers, calculate this span.
