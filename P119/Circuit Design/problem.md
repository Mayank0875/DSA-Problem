...
## Description
An engineer designs nested sub-circuits. '(' starts a module, ')' ends it.

The structure is represented by a **balanced** parentheses string $s$, where `'('` signifies starting a module and `')'` signifies ending a module.

The engineer wants to isolate a signal path that has a nesting depth of exactly $k$. They can choose to skip certain components of the structure, effectively using a **subsequence** of the original string. However, the chosen subsequence must still form a valid, balanced sequence.
