...
## Description
An alchemist adds ingredients to a cauldron one by one.

The ingredients are represented as a string $S$.
However, the order of the items is not fixed. The mixing order affects the potion's stability.

Your task is to calculate the **total number of distinct recipes** that can be formed using the characters of the string $S$. Since the number of possibilities can be very large, print the result modulo $10^9 + 7$.
