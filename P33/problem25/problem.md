...
You are given an array of `n` height values for a roller coaster's first big hill. The track goes up to a single peak and then drops. This is a "mountain array".
    1. n >= 3
    2. `h[0] < h[1] < ... < h[i-1] < h[i]`
    3. `h[i] > h[i+1] > ... > h[n-1]`

Find the index of the hill's highest point.
