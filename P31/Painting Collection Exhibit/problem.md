...
A museum has $N$ paintings, with values $candidates_i$. The collection may include several paintings of the same value. The curator wants to create an exhibit with a total value of exactly $T$. Your task is to find the *total count* of unique combinations of paintings that sum to $T$. Each painting may be used *only once*.
