...
## Description
A weaver measures the thread count in different fabric sections.

The weaver has a sequence of sections lined up in a row. The $i$-th section corresponds to `nums[i]` threads.

The weaver wants to verify if the collection contains a valid "**Pattern Match**." A Pattern Match is defined as a continuous sub-segment of sections that meets two conditions:
1. The sequence must contain **at least two** sections.
2. The total sum of the threads in the sequence must be perfectly divisible by $k$ (the pattern repeat).

Recall that 0 is always a multiple of $k$.

Your task is to determine if such a sequence exists. Return `true` if a Pattern Match is found, and `false` otherwise.
