...
## Description
A solar farm records the energy produced (in kWh) each day. For each day, they calculate a 'production span': the number of consecutive previous days (including the current one) where the energy output was less than or equal to today's output. Given the production data, calculate this span.
