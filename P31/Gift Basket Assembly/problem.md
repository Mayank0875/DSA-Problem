...
You are assembling a gift basket that must have a total value of exactly $T$. You have a collection of $N$ items to choose from, with costs $candidates_i$. This collection may contain multiple items with the same cost. Your task is to find the *total count* of unique combinations of these items that sum to $T$. Each item from the collection may be used *only once*.
