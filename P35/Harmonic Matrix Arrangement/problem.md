...
A composer is designing a harmonic matrix composed of identical resonant cells.  
There are **n** cells, each having a width of **w** units and a height of **h** units.  
To create the matrix, all cells must be placed flat inside a single square frame without rotating any cell.  
Your task is to determine the smallest possible side length of the square frame that can accommodate all **n** cells without overlap.
