...
A network router maintains a priority queue of data packets, sorted by a priority number. A new packet with `target` priority arrives.

Your task is to find the index where this new packet should be inserted into the queue.
The index should point to the first packet in the queue with a priority greater than or equal to `target`.
If all packets in the queue have lower priority, the new packet is placed at the end.
