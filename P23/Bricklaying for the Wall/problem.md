...
A team of `n` masons is building a great wall. They need to lay a total of `t` bricks. Each mason `i` works at their own speed, taking `k[i]` seconds to lay one brick. All masons can work in parallel. What is the shortest time in which the team can lay at least `t` bricks?
