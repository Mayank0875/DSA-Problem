...
A surveyor's map shows land plots in an m x n grid, each with a plot ID. The grid is organized as follows:
    1. Each row is sorted by plot ID.
    2. The first plot ID in any row is greater than the last plot ID in the previous row.

Given a target plot ID, return 1 if it is on the map or 0 otherwise. 
