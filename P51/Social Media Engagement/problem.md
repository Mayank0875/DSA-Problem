...
## Description
A content creator tracks the number of likes on their posts. For each post, they want to determine how many consecutive previous posts (including the current one) performed worse or equal (had fewer or equal likes). This metric helps gauge relative viral success. Given the sequence of like counts, compute this metric for every post.
