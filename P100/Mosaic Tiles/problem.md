...
## Description
An artist places a row of colored tiles for a border.

The tile set is represented as a string $S$.
However, the order of the tiles is not fixed. The artistic value depends on the sequence of colors used.

Your task is to calculate the **total number of distinct borders** that can be formed using the characters of the string $S$. Since the number of possibilities can be very large, print the result modulo $10^9 + 7$.
