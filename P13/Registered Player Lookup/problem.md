...
You are running a gaming tournament. All registered players have a unique ID, and these IDs are stored in a sorted list. When a player arrives, they give you their `target` ID. You need to quickly verify if they are on the registration list. Write a function that takes the sorted list of IDs and a target ID, and confirms their registration status. You must write an algorithm with O(log n) runtime complexity.
