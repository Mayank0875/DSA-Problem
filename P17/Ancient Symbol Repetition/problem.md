...
An archaeologist has found an `n`-symbol inscription, represented by an array of `Symbol IDs`. They theorize that a "pattern key" exists if `any Symbol ID appears at least twice` in the inscription `within a distance of 'k' positions` from its partner. You must verify this theory.

If a pattern key is found, return `true`; otherwise, return `false`.
