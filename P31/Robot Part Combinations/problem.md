...
You are building a robot and have a bin with $N$ parts, with costs $candidates_i$. The bin contains many parts, including duplicates (e.g., five 2-credit bolts). You need to build a robot with a total part cost of exactly $T$. Your task is to find the *total count* of unique combinations of parts from the bin that sum to $T$. Each part may be used *only once*.
