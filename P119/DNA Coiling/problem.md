...
## Description
A biologist models supercoiling. '(' is a twist left, ')' is a twist right.

The structure is represented by a **balanced** parentheses string $s$, where `'('` signifies twisting and `')'` signifies untwisting.

The biologist wants to study the structure that has a nesting depth of exactly $k$. They can choose to skip certain twists of the structure, effectively using a **subsequence** of the original string. However, the chosen subsequence must still form a valid, balanced sequence.
