...
## Description
A router sees `n` possible paths, each with a specific latency cost. To avoid harmonic interference, the router balances load over paths whose costs have a GCD of 1.

Your task:
Count how many non-empty subsequences of paths have a greatest common divisor (GCD) of their latency costs exactly equal to 1.
...
## Input Format
- The first line contains an integer n (the number of paths).
- The second line contains n integers $a_1, a_2, \dots, a_n$ representing the latency costs.
