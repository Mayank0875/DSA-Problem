...
## Description
A robotic arm moves over a grid of bins containing different parts. To assemble a specific gadget, the arm must pick parts in a specific order, moving only to adjacent bins. Determine if the required sequence of parts can be collected from the current bin layout.
