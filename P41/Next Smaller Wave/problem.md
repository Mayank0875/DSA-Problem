...
## Description
An oceanographer is studying wave heights arriving at the shore. For each wave, they want to identify the height of the nearest following wave that is strictly smaller. This helps in analyzing wave set patterns and lull periods. Given the sequence of wave heights, determine the height of the next smaller wave for each one. If all subsequent waves are larger or equal, report that no smaller wave follows.
