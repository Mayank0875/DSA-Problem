...
In a video game, you get points for actions. Given an array `nums` of positive scores from consecutive actions and a `target` score, find the length of the shortest contiguous sequence of actions (subarray) whose total score is greater than or equal to `target`. If no such combo exists, return 0.
