...
You are managing an arcade game's high score leaderboard. The scores are stored in a sorted list, from lowest to highest. A new player has just finished a game with a `target` score.

Your task is to find the correct index to insert this new score into the leaderboard.
The index should be the position of the first score that is greater than or equal to the new score.
If all existing scores are smaller, the new score should be placed at the end.
