...
## Description
Tea leaves settle in a line `s`. As the tea is drunk, leaves shift or vanish. How many unique omens can be read from the remaining leaves?

Your task is to determine the total number of distinct non-empty omens that can be formed. Two omens are considered distinct if they are not identical strings. As the number of possibilities can be substantial, print the answer modulo $10^9 + 7$.
