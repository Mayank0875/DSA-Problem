...
You are given an m x n matrix of magic numbers. This "magic square" isn't a true magic square, but it has special sorting properties:
    1. Each row is sorted.
    2. The first number of each row is greater than the last number of the previous row.

Given a target magic number, return 1 if it is in the square or 0 otherwise. 
