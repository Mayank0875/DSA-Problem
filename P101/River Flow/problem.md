...
## Description
A river flows through checkpoints `s`. Drought dries up certain sections. How many unique flow paths can the water take downstream?

Your task is to determine the total number of distinct non-empty flow paths that can be formed. Two flow paths are considered distinct if they are not identical strings. As the number of possibilities can be substantial, print the answer modulo $10^9 + 7$.
