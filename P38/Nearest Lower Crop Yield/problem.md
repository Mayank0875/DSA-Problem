...
## Description
You are an agricultural scientist analyzing the annual crop yield of a farm over several decades. To understand long-term productivity improvements, you want to find the yield of the nearest previous year that was strictly lower than the current year's yield. This comparison allows you to see how the current harvest compares to recent poor harvests. For each year's yield data, you need to determine the value of the most recent prior year that had a strictly lower yield. If no such year exists because previous yields were always higher or equal, you must output a specific indicator. Your task is to perform this analysis for the entire dataset.
