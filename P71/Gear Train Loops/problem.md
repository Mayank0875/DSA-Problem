...
## Description
Gears are arranged in loops to transfer power. Each gear meshes with two others.

There are n gears.
Loops have at least 2 gears.
Diagrams show one mesh point for each gear.
...
5 <-> 6.
Maximum:
These could be 3 separate loops of 2 gears each.
Minimum:
Since we only know one neighbor per gear, these pairs could actually be segments of a bigger ring. All gears can be arranged into one single ring:
1 - 2 - 3 - 4 - 5 - 6 - 1.
...
- The second line contains n space-separated integers a1, a2, ..., an, where ai is the neighbor remembered by the i-th gear.
