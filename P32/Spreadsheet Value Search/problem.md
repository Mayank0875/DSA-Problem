...
You are given a large spreadsheet as an m x n matrix of integers. The data has been sorted with the following properties:
    1. Each row is sorted from left to right.
    2. The first cell in each row is greater than the last cell in the previous row.

Given a target value, return 1 if the value is in the spreadsheet or 0 otherwise. 
