...
## Description
A hacker $x$ jacks into deck $y$. The link stabilizes if the synaptic lag (XOR) divides the hacker's level.

The netrunner defines a specific relationship between a fixed integer $x$ and a candidate integer $y$. A value $y$ is considered "**stable**" with $x$ if:
