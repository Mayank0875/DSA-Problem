...
## Description
A weather station records rainfall in millimeters for consecutive rain showers. For each shower, meteorologists calculate a 'dryer-streak': the number of consecutive previous showers (including the current one) that had less or equal rainfall volume. Given the rainfall data, calculate this streak for each event.
