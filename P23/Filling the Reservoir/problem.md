...
A large reservoir needs to be filled with `t` liters of water. There are `n` water pumps available. Each pump `i` has a different flow rate, taking `k[i]` seconds to pump exactly one liter of water. All pumps can operate at the same time. What is the minimum time needed to fill the reservoir with at least `t` liters?
