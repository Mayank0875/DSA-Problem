...
Previous Stronger Loopstorm Intensity

## Slug

previous-stronger-loopstorm
...
Imagine monitoring a sequence of loopstorms, each characterized by its intensity. As you observe each loopstorm in order, you look back (to earlier storms) and want to know the intensity of the nearest previous loopstorm that is strictly stronger than the current one. If all earlier loopstorms are weaker or equal, or if it is the first storm, there is no such stronger storm to the left. Your goal is to determine this stronger storm's intensity for each position in the sequence.
