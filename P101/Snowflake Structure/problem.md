...
## Description
A snowflake grows branches `s`. Melting reduces the branches. How many unique crystal shapes can the snowflake transition into?

Your task is to determine the total number of distinct non-empty shapes that can be formed. Two shapes are considered distinct if they are not identical strings. As the number of possibilities can be substantial, print the answer modulo $10^9 + 7$.
