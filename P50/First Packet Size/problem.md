...
## Description
A network sniffer logs packet sizes sorted by size. A security analyst is looking for the first packet that matches a specific byte size associated with a known exploit. Given the sorted packet sizes, find the index of the first match.
