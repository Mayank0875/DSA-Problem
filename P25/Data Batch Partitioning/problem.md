...
A large data stream is broken into `n` sequential chunks, with chunk `i` having size `x[i]`. This stream must be processed by `k` parallel workers. Each worker must be assigned a contiguous block of chunks (e.g., worker 1 gets chunks 1-2, worker 2 gets 3-5, etc.). What is the minimum possible value for the maximum data load (total size) given to any single worker?
...
- The first line contains two integers `n` and `k`: the number of chunks and the number of workers.
- The second line contains `n` integers `x[1], x[2] ... x[n]`, where `x[i]` is the size of the i-th chunk.
