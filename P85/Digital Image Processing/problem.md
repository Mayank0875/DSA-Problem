...
## Description
An algorithm processes `n` pixels with specific color values. A filter is applied to a region if the GCD of the pixel values in that region is 1.

Your task:
Count how many non-empty subsequences of pixels have a greatest common divisor (GCD) of their color values exactly equal to 1.
...
## Input Format
- The first line contains an integer n (the number of pixels).
- The second line contains n integers $a_1, a_2, \dots, a_n$ representing the color values.
