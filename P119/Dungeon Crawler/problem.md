...
## Description
An adventurer explores a multi-floor dungeon. '<' goes down stairs, '>' goes up stairs.

The structure is represented by a **balanced** parentheses string $s$, where `'<'` signifies descending stairs and `'>'` signifies ascending stairs.

The hero wants to raid a specific floor depth that has a nesting depth of exactly $k$. They can choose to skip certain floors of the structure, effectively using a **subsequence** of the original string. However, the chosen subsequence must still form a valid, balanced sequence.
