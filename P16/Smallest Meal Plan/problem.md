...
Given an array `nums` of positive calorie counts for consecutive meals, and a `target` calorie goal, find the length of the shortest contiguous sequence of meals (subarray) whose total calories are greater than or equal to `target`. If no such sequence exists, return 0.
