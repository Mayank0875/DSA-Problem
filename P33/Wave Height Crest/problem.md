...
The profile of a single ocean wave is measured at `n` points. The height increases to a crest and then decreases as the wave breaks. This forms a "mountain array":
    1. n >= 3
    2. `h[0] < h[1] < ... < h[i-1] < h[i]`
    3. `h[i] > h[i+1] > ... > h[n-1]`

Find the index of the wave's crest.
