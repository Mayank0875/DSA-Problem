...
A group of `n` dwarves is mining for gems. They need to find a total of `t` gems. Each dwarf `i` has their own technique, taking `k[i]` seconds to find and extract one gem. All dwarves can mine simultaneously. What is the shortest time for the group to find at least `t` gems?
