...
A data stream is received as an array A of N bits (0s and 1s). To ensure integrity, you must perform Q checks. Each check (query) is defined by a start index L, an end index R, and a bit value `Val`. You must count the occurrences of `Val` in the subarray A[L .. R]. The final checksum is the XOR of all Q counts.
