...
## Description
A probe descends through atmospheric layers. '(' enters a layer, ')' exits it.

The structure is represented by a **balanced** parentheses string $s$, where `'('` signifies entering a layer and `')'` signifies leaving a layer.

The probe wants to measure a profile that has a nesting depth of exactly $k$. They can choose to skip certain gas layers of the structure, effectively using a **subsequence** of the original string. However, the chosen subsequence must still form a valid, balanced sequence.
