...
## Description
A pyrotechnician loads shells into a mortar rack.

The shells are represented as a string $S$.
However, the order of the shells is not fixed. The show's finale depends on the sequence of explosions.

Your task is to calculate the **total number of distinct finales** that can be formed using the characters of the string $S$. Since the number of possibilities can be very large, print the result modulo $10^9 + 7$.
