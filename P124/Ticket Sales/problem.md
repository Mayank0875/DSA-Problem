...
## Description
A box office manager tracks tickets sold per hour.

The manager has a sequence of hours lined up in a row. The $i$-th hour corresponds to `nums[i]` tickets.

The manager wants to analyze if the collection contains a valid "**Row Fill**." A Row Fill is defined as a continuous sub-segment of hours that meets two conditions:
1. The sequence must contain **at least two** hours.
2. The total sum of the tickets in the sequence must be perfectly divisible by $k$ (the seating row capacity).

Recall that 0 is always a multiple of $k$.

Your task is to determine if such a sequence exists. Return `true` if a Row Fill is found, and `false` otherwise.
