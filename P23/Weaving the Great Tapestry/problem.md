...
A guild of `n` weavers is creating a great tapestry. The tapestry requires `t` meters of thread to be woven. Each weaver `i` has a different skill, taking `k[i]` seconds to weave one meter of thread. All weavers can work at the same time. What is the minimum time they need to complete the `t` meters?
