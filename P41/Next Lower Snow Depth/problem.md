...
## Description
A ski resort tracks snow depth daily. To analyze melting patterns, they want to find the snow depth of the nearest future day that is strictly lower than the current depth. Given the sequence of snow depth measurements, find the next lower value for each day. If snow depth increases or stays the same (due to fresh snow), output -1.
