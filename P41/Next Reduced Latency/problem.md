...
## Description
A network engineer is tracking ping times to a server. For each ping measurement, they want to find the latency of the nearest future ping that is strictly lower (faster). This helps in identifying when network lag spikes subside. Given the sequence of latency values in milliseconds, determine the next lower latency for each measurement. If latency remains high, report -1.
