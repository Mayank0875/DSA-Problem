## Title
Knight's Cipher Quest

## Slug
knights-cipher-quest

## Difficulty
Medium

## Description
A medieval knight finds a stone tablet with letters carved into a grid. A legend says that if the name of the ancient king can be traced on the tablet by moving only to adjacent squares (up, down, left, right), the hidden door will open. You are given the grid of letters and the king's name. Determine if the door can be opened.
