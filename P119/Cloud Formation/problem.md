...
## Description
A weather balloon rises. '(' enters a cloud, ')' exits.

The structure is represented by a **balanced** parentheses string $s$, where `'('` signifies entering and `')'` signifies exiting.

The balloon wants to sample moisture that has a nesting depth of exactly $k$. They can choose to skip certain clouds of the structure, effectively using a **subsequence** of the original string. However, the chosen subsequence must still form a valid, balanced sequence.
