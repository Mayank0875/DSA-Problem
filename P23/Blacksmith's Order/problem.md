...
A king has ordered `t` swords for his army. The royal forge has `n` blacksmiths. Each blacksmith `i` is a master of their craft but works at a different speed, taking `k[i]` seconds to forge a single sword. All blacksmiths can work in parallel. What is the minimum time required to fulfill the king's order of `t` swords?
