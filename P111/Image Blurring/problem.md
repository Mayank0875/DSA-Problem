...
## Description
In image processing, a 1D blur filter averages the pixel intensity of a point with its surrounding pixels to reduce sharpness.

You are given a **0-indexed** array `nums` of `n` integers representing pixel intensities and an integer `k`.
...
## Input Format
- The first line contains an integer `n`, representing the number of pixel intensities.
