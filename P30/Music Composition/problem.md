...
You are composing a bar of music that must have a total duration of $T$ beats. You can use different types of notes, each with a beat duration (e.g., 2, 3, 5). You can use any note type an unlimited number of times. Your task is to find the *total number* of unique combinations of notes that will sum exactly to $T$ beats.
