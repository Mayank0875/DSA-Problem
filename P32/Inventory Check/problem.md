...
You are checking an inventory sheet, which is an m x n matrix of item IDs. The sheet is sorted by ID with the following rules:
    1. Each row is sorted in non-decreasing order.
    2. The first ID in a row is greater than the last ID in the previous row.

Given a target item ID, return 1 if the item is on the sheet or 0 otherwise. 
