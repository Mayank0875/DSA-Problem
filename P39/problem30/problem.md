...
Imagine exploring a corridor of ancient digital ruins, where each terminal holds a data fragment of a certain size. As you stand at each terminal, you look back (to your left) and want to know the size of the nearest fragment that is strictly larger than the one you are currently at. If all previous fragments are smaller or of equal size, or if it's the first terminal, there's no such larger fragment to the left. Your goal is to determine this larger fragment's size for each position along the corridor.
