...
A team of explorers must create at least n identical maps of a mysterious labyrinth before night falls. They start with a single original map. Two enchanted copying stations are available: the first one produces a copy in x seconds, and the second one in y seconds. A station can copy either the original map or any copy that already exists, and both stations may operate at the same time.  
Determine the minimum amount of time required to have at least n additional maps, starting from just the original.
