...
## Description
A chef peels a giant genetically modified onion. '(' denotes a layer starting and ')' denotes it ending.

The structure is represented by a **balanced** parentheses string $s$, where `'('` signifies layer start and `')'` signifies layer end.

The chef wants to prepare a dish that has a nesting depth of exactly $k$. They can choose to skip certain layers of the structure, effectively using a **subsequence** of the original string. However, the chosen subsequence must still form a valid, balanced sequence.
