...
## Description
A robot's distance sensor is jittery. The navigation system smooths the input stream by averaging readings within a specific time radius.

You are given a **0-indexed** array `nums` of `n` integers representing sensor readings and an integer `k`.
...
## Input Format
- The first line contains an integer `n`, representing the number of sensor readings.
