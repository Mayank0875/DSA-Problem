...
Imagine you are constructing a synthetic sequence of numbers. As you examine each element of the sequence, you look back (to the left) and want to know the value of the nearest earlier element that is strictly larger than the current one. If all previous elements are smaller or equal, or if it is the first element, there is no such larger element to the left. Your goal is to determine this larger element’s value for every position in the sequence.
