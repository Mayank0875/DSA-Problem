...
You have a list of N tasks for the day, stored in array A (0 = Incomplete, 1 = Complete). You need to answer Q queries from your manager. Each query (L, R, Val) asks for the number of tasks in the range L to R that have the status `Val`. After processing all queries, you must report the XOR sum of their answers as a verification code.
