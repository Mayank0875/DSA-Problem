...
## Description
A group admin tracks the number of messages sent in a chat group daily. For each day, they calculate a 'volume span': the number of consecutive previous days (including the current one) where the message count was less than or equal to today's count. Given the daily message logs, calculate this span.
