...
## Description
A chef has a pantry full of ingredients (represented by characters) and a book of recipes (strings). A recipe can be cooked if all its required ingredients are present in the pantry. Calculate the total 'complexity' of all cookable recipes, where complexity is the number of ingredients (length of the string).
