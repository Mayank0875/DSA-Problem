...
## Description
A developer tracks their daily coffee consumption (in cups). For each day, they want to calculate a 'caffeine streak', which is the number of consecutive previous days (including the current one) where they drank fewer or equal cups compared to today. Given the daily logs, calculate this streak.
