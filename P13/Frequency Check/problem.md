...
An audio engineer is working with a set of sound frequencies, all sorted in a list. They need to check if a specific `target` frequency is included in their set. You must write an extremely fast function to perform this check, given the sorted list and the target frequency. You must write an algorithm with O(log n) runtime complexity.
