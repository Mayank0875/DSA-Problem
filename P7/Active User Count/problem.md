...
A website tracks its N users by their activity (0 = inactive, 1 = active) in an array A. You are given Q queries to analyze user engagement. Each query (L, R, Val) asks for the number of users from L to R (inclusive) who have the activity status `Val`. To validate the analysis, you must output the XOR of all query results.
