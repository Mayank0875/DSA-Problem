...
You are exploring a vast parallel labyrinth where each chamber has a specific height. For each chamber you enter, you want to find the height of the nearest previous chamber (in the order you traversed) that had a strictly lower height than the current chamber. If no such previous chamber exists (either because it's the first chamber or all earlier chambers were higher or equal), you should indicate this. This analysis helps map the descent patterns within the labyrinth. Your task is to compute this value for every chamber given the sequence of chamber heights.
