...
## Description
A chef uses exactly k ingredients from the pantry for a stew. Ingredients are identified by ID. To simplify future inventory, the chef uses up ingredients that are low in stock to remove them from the list. Find the minimum number of unique ingredient types left.
