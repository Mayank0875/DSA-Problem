...
## Description
You are testing a CPU cooling system. You log the temperature every second. To measure cooling latency, you want to find the temperature of the nearest future second that is strictly cooler than the current second. Given the list of temperature readings, identify the next lower temperature for each point in time. If the CPU temperature never drops below the current value during the rest of the test, output -1.
