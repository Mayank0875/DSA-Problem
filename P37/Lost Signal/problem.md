...
A remote outpost has lost its communication signal. The central hub holds the original signal packet and must broadcast exactly **n** additional packets before the next transmission window opens. Two transmitters are available.
The first transmitter needs **x** seconds to send one packet (it can retransmit the original or any already‑sent packet), and the second transmitter needs **y** seconds. The operators may use either transmitter alone or both at the same time.  
Help the hub determine the minimum amount of time required to produce at least **n** new signal packets, starting with just the original one.
