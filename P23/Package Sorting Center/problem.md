...
A mail sorting center has `n` sorting machines. A truck has just arrived with `t` packages. Each machine `i` can sort packages at a different speed, taking `k[i]` seconds per package. All machines can operate simultaneously. What is the minimum time required to sort all `t` packages?
