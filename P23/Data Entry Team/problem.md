...
A data entry team of `n` typists needs to input `t` records into a new system. Each typist `i` works at a different speed, taking `k[i]` seconds per record. If all typists start working at the same time, what is the minimum time required for the team to input at least `t` records?
