...
You are given a sorted list of code commit IDs. A new commit with ID `target` has just been made, and you need to add it to the log.

Your task is to find the index where this new commit ID should be inserted to maintain sorted order.
The index should be the first position with a commit ID greater than or equal to `target`.
If all logged commits have smaller IDs, the new commit goes at the end.
