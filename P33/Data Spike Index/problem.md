...
A data stream of `n` values shows a sudden spike and then a return to normal. This pattern forms a "mountain array":
    1. n >= 3
    2. `val[0] < val[1] < ... < val[i-1] < val[i]`
    3. `val[i] > val[i+1] > ... > val[n-1]`

Your task is to find the index of the highest value in this data spike.
