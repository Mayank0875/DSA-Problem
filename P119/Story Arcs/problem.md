...
## Description
A writer structures a novel. '(' starts a subplot, ')' resolves it.

The structure is represented by a **balanced** parentheses string $s$, where `'('` signifies plot start and `')'` signifies plot resolution.

The writer wants to create a narrative depth that has a nesting depth of exactly $k$. They can choose to skip certain chapters of the structure, effectively using a **subsequence** of the original string. However, the chosen subsequence must still form a valid, balanced sequence.
