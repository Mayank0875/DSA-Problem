...
## Description
A pedometer counts steps per minute. To encourage the user, it displays a smoothed activity level metric.

You are given a **0-indexed** array `nums` of `n` integers representing step counts and an integer `k`.
...
## Input Format
- The first line contains an integer `n`, representing the number of step counts.
