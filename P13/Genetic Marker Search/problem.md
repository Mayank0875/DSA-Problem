...
A biologist is studying a set of genetic markers, identified by unique numbers. These marker IDs are stored in a large, sorted database. The biologist needs to check if a new marker, with a `target` ID, has already been cataloged. You must write an efficient function to perform this lookup, taking the sorted list and target ID as input. You must write an algorithm with O(log n) runtime complexity.
