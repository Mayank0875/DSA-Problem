...
In a quantum cascade laboratory, researchers need to arrange a set of identical rectangular energy packets within a square containment field.  
There are **n** packets, each having a width of **w** units and a height of **h** units. The packets must be placed flat (no rotation allowed) inside a single square field.  
Determine the smallest possible side length of the square field that can accommodate all **n** packets without any overlap.
