...
Data Rift: Node Activation Grid
...
A data rift contains n data nodes arranged in a line, numbered 1 to n. Initially, all nodes are active.  
The rift runs n cycles. For the i‑th cycle (where i goes from 1 to n), every node whose number j is a multiple of i toggles its state.  
An active node becomes inactive, and an inactive node becomes active. After all n cycles are completed, the rift observes the final state of the nodes. The rift needs exactly k nodes to be active in the end. Find the smallest possible number of nodes n the rift should start with to achieve exactly k active nodes.
