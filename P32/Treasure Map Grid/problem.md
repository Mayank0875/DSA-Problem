...
You have a treasure map laid out as an m x n grid. Each cell has a "clue number". The map is magical and its clue numbers obey two rules:
    1. Each row is sorted by clue number, left-to-right.
    2. The first clue number in any row is greater than the last clue number in the previous row.

Given a target clue number, return 1 if it's on the map or 0 otherwise. 
