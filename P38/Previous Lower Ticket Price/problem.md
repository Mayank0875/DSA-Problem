...
## Description
You are analyzing airline ticket prices for a specific route, checked daily leading up to the flight. You have a list of prices recorded each day. To assist travelers in understanding price volatility, you want to find the price on the nearest previous day that was strictly lower than the current day's price. This helps identify if the current price is a local high point. For each recorded price, your goal is to find the value of the closest preceding day where the price was strictly cheaper. If all previous prices were higher or equal, you must indicate that no lower price exists. Your task is to process the ticket price history.
