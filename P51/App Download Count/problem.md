...
## Description
A mobile developer tracks daily app downloads. For each day, they calculate a 'download streak', defined as the number of consecutive previous days (including the current one) where the download count was less than or equal to today's count. Given the daily download figures, calculate this streak for every day.
