...
## Description
An operating system manages memory blocks. The addresses of free memory blocks are kept in a sorted list. A process requests memory starting at a preferred address. The memory manager needs to find the index of the first free block that is located at or after the preferred address.
