...
## Description
A pyrotechnician sets a firing sequence `s`. Some fuses fail to light. How many unique shows can the audience witness?

Your task is to determine the total number of distinct non-empty shows that can be formed. Two shows are considered distinct if they are not identical strings. As the number of possibilities can be substantial, print the answer modulo $10^9 + 7$.
