...
## Description
A sysadmin is matching server cluster capacities in two different regions. Capacities can be upgraded by integer factors (doubling, tripling, etc.).

You have two lists of server capacities.

In a single upgrade, The sysadmin can:
1. Choose an index $i$ ($1 \le i \le n$).
2. Choose a positive integer $x$.
3. scale the capacity value at $i$ in either list by $x$ (i.e., new value = old value $\times x$).

The sysadmin wants to make the lists identical (i.e., $a_i = b_i$ for all $1 \le i \le n$) using the **minimum** number of upgrades.
