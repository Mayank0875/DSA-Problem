...
## Description
Biologists are studying an ant colony. The ground is divided into a grid, each cell marked with a scent marker. They want to know if a scout ant could have traveled a specific path defined by a sequence of scents, moving only to adjacent cells. Verify if the path is possible.
