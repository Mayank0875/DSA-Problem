...
You are analyzing a sound wave. The amplitude over a short time interval is given by an array of `n` measurements. The wave builds to a crest and then falls, forming a "mountain array":
    1. n >= 3
    2. `amp[0] < amp[1] < ... < amp[i-1] < amp[i]`
    3. `amp[i] > amp[i+1] > ... > amp[n-1]`

Find the index of the wave's crest (the peak amplitude).
