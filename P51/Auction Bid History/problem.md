...
## Description
In a live auction, bids are placed sequentially. For each bid, the auctioneer notes the 'dominance streak', which is the number of consecutive preceding bids (including the current one) that were less than or equal to the current bid amount. Given the list of bids, calculate this streak for each one.
