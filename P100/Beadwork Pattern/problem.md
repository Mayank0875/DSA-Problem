...
## Description
An artisan threads beads onto a bracelet.

The beads are represented as a string $S$.
However, the order of the beads is not fixed. The pattern is defined by the sequence of bead types.

Your task is to calculate the **total number of distinct bracelets** that can be formed using the characters of the string $S$. Since the number of possibilities can be very large, print the result modulo $10^9 + 7$.
