...
## Description
A warehouse has shelves at various heights, sorted from lowest to highest. A forklift driver needs to store a pallet of a specific height. To fit it, they must find the first shelf that provides enough clearance (height >= pallet height). Return the index of that shelf.
