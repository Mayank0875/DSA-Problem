...
Imagine a mysterious loop that disappears each time you step into it. As you move along a sequence of numbers, at each position you look back to the numbers that were still present before the loop vanished and want to know the height of the nearest previous number that is strictly larger than the current one. If all earlier numbers are smaller or equal, or if you are at the first position, there is no such larger number to the left. Your task is to determine this larger number’s value for every position in the sequence.
