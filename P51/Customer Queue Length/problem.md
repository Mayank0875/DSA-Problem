...
## Description
A bank manager records the maximum length of the customer queue every hour. For each hour, they calculate a 'queue span': the number of consecutive previous hours (including the current one) where the queue length was less than or equal to the current length. Given the queue length data, calculate this span for each hour.
