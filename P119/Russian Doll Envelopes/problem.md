...
## Description
You fit envelopes inside envelopes. '(' is an open flap, ')' is closed.

The structure is represented by a **balanced** parentheses string $s$, where `'('` signifies inserting and `')'` signifies sealing.

The clerk wants to organize mail that has a nesting depth of exactly $k$. They can choose to skip certain envelopes of the structure, effectively using a **subsequence** of the original string. However, the chosen subsequence must still form a valid, balanced sequence.
