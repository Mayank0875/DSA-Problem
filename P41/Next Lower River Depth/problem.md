...
## Description
During a flood event, sensors record river depth hourly. To track receding waters, you want to find the depth at the nearest future hour that is strictly lower than the current depth. Given the sequence of depth measurements, determine the next lower depth for each hour. If the water level continues to rise or stays steady, report that no lower depth exists.
