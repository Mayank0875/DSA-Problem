...
Given an array `nums` of positive page counts for books on a shelf, and a `target` page count, find the length of the shortest contiguous block of books (subarray) whose total pages are greater than or equal to `target`. This is for a reading challenge. If no such block exists, return 0.
