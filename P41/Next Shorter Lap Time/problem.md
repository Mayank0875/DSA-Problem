...
## Description
A racing team is analyzing lap times. For each lap, they want to find the time of the nearest future lap that was strictly faster (lower time value). Note: Lower is better/faster in racing. Given the sequence of lap times, find the next lower time value for each lap. If the driver never beats the current lap time in subsequent laps, output -1.
