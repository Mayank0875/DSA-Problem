...
## Description
A dishwasher is stacking plates. Clean plates (0) go on top (left in array representation) of Dirty plates (1).

We can represent the arrangement as a binary string `stack` of length `n`, consisting of '0's (Clean plates) and '1's (Dirty plates). A string is considered **sorted** if all '0's appear before all '1's.
...
Your task is to calculate the **minimum number of operations** required to make the string `stack` sorted (i.e., all Clean platess precede all Dirty platess).
...
#### Explanation
We need to move the Clean platess (0) that appear after Dirty platess (1).
...
#### Explanation
There are three distinct groups of '0's (Clean plates) that appear after the first '1' (Dirty plates):
