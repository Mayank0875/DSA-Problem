...
An encrypted message is hidden in an m x n grid of numbers. The numbers are not random; they are sorted:
    1. Each row is sorted in non-decreasing order.
    2. The first number of each row is greater than the last number of the previous row.

Given a target number (the key), return 1 if the key is in the grid or 0 otherwise. 
