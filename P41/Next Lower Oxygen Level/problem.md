...
## Description
You are monitoring dissolved oxygen levels in a fish tank. For each measurement, you need to alert if there is a drop, but specifically, you want to analyze the data by finding the nearest future measurement that is strictly lower than the current one. Given the sequence of oxygen readings, find the next lower value for each. If oxygen levels remain stable or rise, output -1.
