...
## Description
You are a coach analyzing the split times of a marathon runner. A split time is recorded every kilometer. To analyze pacing strategy, you want to find the time of the nearest previous split that was strictly faster (lower time value) than the current split. Note: In this context 'lower' means a smaller time value (faster). For each split time in the sequence, your goal is to find the value of the closest preceding split that was strictly smaller (faster). If all previous splits were slower or equal, you must indicate this. Your task is to analyze the runner's split times for the whole race.
