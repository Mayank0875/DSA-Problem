...
## Description
A patron finds a seat. '(' enters a tier, ')' leaves it.

The structure is represented by a **balanced** parentheses string $s$, where `'('` signifies entering and `')'` signifies leaving.

The patron wants to get a view that has a nesting depth of exactly $k$. They can choose to skip certain tiers of the structure, effectively using a **subsequence** of the original string. However, the chosen subsequence must still form a valid, balanced sequence.
