...
## Description
Neurons fire in reverberating loops.

There are n neurons.
Loops have at least 2 neurons.
Each neuron activates one other.
...
5 <-> 6.
Maximum:
These could be 3 separate loops of 2 neurons each.
Minimum:
Since we only know one neighbor per neuron, these pairs could actually be segments of a bigger ring. All neurons can be arranged into one single ring:
1 - 2 - 3 - 4 - 5 - 6 - 1.
...
- The second line contains n space-separated integers a1, a2, ..., an, where ai is the neighbor remembered by the i-th neuron.
