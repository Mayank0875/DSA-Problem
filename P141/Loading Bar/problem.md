...
## Description
A circular loading indicator has segments. Some are filled ('1'). The animation logic fills the segment $d$ steps ahead.
...
You want to transform the system so that all elements become filled. You have a special animation step that allows you to perform an operation. In one operation, you can choose a shift amount $d$ ($1 \le d \le n$). This operation spreads the filled state cyclically to the right by $d$ positions. Specifically, if an element at index $i$ is filled, the element at index $(i+d) \% n$ also becomes filled (if it wasn't already).

The cost of choosing shift $d$ is $d$ GPU cycles. You can perform this operation any number of times with different $d$. Note that once an element becomes filled, it remains filled.
