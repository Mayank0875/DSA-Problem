...
A neuroscientist has isolated a critical neural pattern and must generate exactly n additional replicas before the experiment begins. Two neural replication modules are available.  
The first module creates a replica in x seconds (it can copy from the original pattern or any existing replica), and the second module creates a replica in y seconds. The modules can be used independently or simultaneously.  
Determine the minimum amount of time required to produce at least n new replicas of the neural pattern, starting with only the original.
