...
## Description
In Beijing, cars with plate numbers up to `P` are restricted. You can drive if your plate digit sum is divisible by today's number `D`.

To check eligibility, you must calculate the count of plates between $1$ and $P$ (inclusive) that satisfy a specific condition: the sum of their digits (in base 10) must be perfectly divisible by $D$.
