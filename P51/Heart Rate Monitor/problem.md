...
## Description
A fitness tracker records heart rate readings every minute. For each reading, the device calculates a 'stability span', which is the number of consecutive preceding minutes (including the current one) where the heart rate was less than or equal to the current reading. Given the array of heart rates, calculate this span for each minute.
