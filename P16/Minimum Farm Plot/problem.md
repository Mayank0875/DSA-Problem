...
Given an array `nums` of positive crop yields per plot in a row, and a positive integer `target` yield, find the length of the shortest contiguous row of plots (subarray) whose total yield is greater than or equal to `target`. If no such row exists, return 0.
