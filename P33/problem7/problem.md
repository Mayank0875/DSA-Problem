...
You are an economist analyzing an asset bubble. The asset's value over `n` months is given. The value increases to a peak and then crashes, forming a "mountain array".
    1. n >= 3
    2. `val[0] < val[1] < ... < val[i-1] < val[i]`
    3. `val[i] > val[i+1] > ... > val[n-1]`

Find the index (month) when the asset's value was at its absolute maximum before the crash.
