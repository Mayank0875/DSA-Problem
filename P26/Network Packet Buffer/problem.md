...
A network router has `n` data packets in its queue, each with a different size. The router needs to send a batch of packets, and the total size of the batch must be *exactly* `sum` to fill a data frame perfectly. Can the router select a subset of the queued packets to achieve this exact sum? Each packet can be selected only once.
## Examples
