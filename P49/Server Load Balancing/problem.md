...
## Description
A load balancer monitors server capacities, which are sorted in a list. An incoming request requires a server with at least a specific amount of capacity. To distribute the load, the system looks for the first server in the sorted list that meets or exceeds the required capacity.
