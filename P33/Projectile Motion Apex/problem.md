...
You are given an array of `n` height measurements for a projectile. The path is a "mountain array": the height strictly increases to an apex, and then strictly decreases.
    1. n >= 3
    2. `h[0] < h[1] < ... < h[i-1] < h[i]`
    3. `h[i] > h[i+1] > ... > h[n-1]`

Find the index of the projectile's apex (its highest point).
