...
## Description
A smart watch records instantaneous pace. To show the runner a steady metric, it displays the average pace centered on the current second.

You are given a **0-indexed** array `nums` of `n` integers representing pace values and an integer `k`.
...
## Input Format
- The first line contains an integer `n`, representing the number of pace values.
