...
## Description
A support system handles tickets with priority numbers. The active queue is sorted by priority. A new high-priority issue comes in with a specific priority value. To maintain order, the system needs to find the correct index to insert this new ticket such that it comes before any existing tickets of equal priority (or after lower priorities).
