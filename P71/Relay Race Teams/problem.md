...
## Description
Runners pass batons in continuous loops for practice.

There are n runners.
Loops have at least 2 runners.
Each runner knows who they pass to.
...
5 <-> 6.
Maximum:
These could be 3 separate loops of 2 runners each.
Minimum:
Since we only know one neighbor per runner, these pairs could actually be segments of a bigger ring. All runners can be arranged into one single ring:
1 - 2 - 3 - 4 - 5 - 6 - 1.
...
- The second line contains n space-separated integers a1, a2, ..., an, where ai is the neighbor remembered by the i-th runner.
