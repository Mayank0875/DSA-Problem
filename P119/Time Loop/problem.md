...
## Description
A traveler gets stuck in loops. '{' starts a loop, '}' ends it.

The structure is represented by a **balanced** parentheses string $s$, where `'{'` signifies loop start and `'}'` signifies loop reset.

The traveler wants to reach the core timeline that has a nesting depth of exactly $k$. They can choose to skip certain iterations of the structure, effectively using a **subsequence** of the original string. However, the chosen subsequence must still form a valid, balanced sequence.
