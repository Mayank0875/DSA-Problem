...
## Description
An engineer monitors energy output from a solar array hourly.

The engineer has a sequence of hours lined up in a row. The $i$-th hour corresponds to `nums[i]` watts.

The engineer wants to analyze if the collection contains a valid "**Charge Cycle**." A Charge Cycle is defined as a continuous sub-segment of hours that meets two conditions:
1. The sequence must contain **at least two** hours.
2. The total sum of the watts in the sequence must be perfectly divisible by $k$ (the battery cell capacity).

Recall that 0 is always a multiple of $k$.

Your task is to determine if such a sequence exists. Return `true` if a Charge Cycle is found, and `false` otherwise.
