...
A wizard's spellbook lists all valid potion ingredients by their unique ID, in sorted order. Before brewing, the wizard needs to check if a `target` ingredient ID is on this list. You must write a function for the wizard's magical assistant (you) to perform this check instantly. You must write an algorithm with O(log n) runtime complexity.
