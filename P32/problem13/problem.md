...
You are querying a database that is returned as an m x n matrix. The database guarantees that its results have the following properties:
    1. Each row is sorted in non-decreasing order.
    2. The first entry of each row is greater than the last entry of the previous row.

Given a target value, return 1 if the target is in the matrix or 0 otherwise. 
