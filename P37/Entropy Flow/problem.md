...
In a thermodynamic laboratory, a researcher must produce exactly **n** additional units of entropy before the experiment begins. The lab contains two entropy generators.  
The first generator creates one unit of entropy in **x** seconds (using either the original source or any already‑generated unit), and the second generator does the same in **y** seconds. The researcher may operate either generator alone or both simultaneously.  
Determine the minimum amount of time required to have at least **n** new units of entropy, starting with just the original source.
