...
A bacterial colony's population is measured over `n` hours. The population grows to a maximum size and then declines as resources run out. This data forms a "mountain array":
    1. n >= 3
    2. `pop[0] < pop[1] < ... < pop[i-1] < pop[i]`
    3. `pop[i] > pop[i+1] > ... > pop[n-1]`

Find the index (hour) at which the population was at its peak.
