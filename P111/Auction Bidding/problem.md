...
## Description
An online auction tracks bid amounts. To estimate the item's true market value, the system averages bids in a local window.

You are given a **0-indexed** array `nums` of `n` integers representing bid amounts and an integer `k`.
...
## Input Format
- The first line contains an integer `n`, representing the number of bid amounts.
