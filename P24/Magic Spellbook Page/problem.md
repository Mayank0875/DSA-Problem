...
A wizard's spellbook has its spells sorted by power level (an integer). A new spell with power `target` has been learned and must be added to the book.

Your task is to find the index where this new spell should be written.
The index should point to the first spell in the book that has a power level greater than or equal to `target`.
If all spells in the book are weaker, the new spell is written at the end.
