...
## Description
An oil company decommissions a pipeline of $n$ segments. The pipe must be cut into individual pieces for transport.

The separation process involves splitting the pipeline or any of its resulting fragments into two parts. You can choose any fragment consisting of more than one segment and split it at any valid boundary between segments. However, the process consumes resources. The cost of a single split is equal to the sum of the weight of all segments in the fragment being split.

Your task is to determine the minimum total cost required to break the entire pipeline down into single isolated segments.
...
- The second line contains $n$ integers representing the weight of each segment.
