...
## Description
You are analyzing a stream of product prices over the upcoming festive season. For every day's price, you want to know the price on the nearest future day that will be strictly cheaper than the current price. This helps customers decide whether to buy now or wait for a better deal shortly. Given a list of predicted daily prices, your task is to determine the value of the next strictly lower price for each day. If prices only go up or stay the same for the rest of the period, you should indicate that no better deal is coming.
