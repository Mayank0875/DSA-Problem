...
## Description
Train $x$ shares track with $y$. Collision avoidance works if the time slot delta (XOR) divides the train ID.

The scheduler defines a specific relationship between a fixed integer $x$ and a candidate integer $y$. A value $y$ is considered "**safe**" with $x$ if:
