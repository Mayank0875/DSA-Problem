...
## Description
A loader organizes the hold. '[' pushes cargo, ']' pops cargo.

The structure is represented by a **balanced** parentheses string $s$, where `'['` signifies loading and `']'` signifies unloading.

The loader wants to organize the inventory that has a nesting depth of exactly $k$. They can choose to skip certain containers of the structure, effectively using a **subsequence** of the original string. However, the chosen subsequence must still form a valid, balanced sequence.
