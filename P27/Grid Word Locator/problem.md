...
You are given a `board` (grid) of letters, similar to a word search puzzle, and a `word`. However, the `word` is not in a straight line. It can be formed by a path of adjacent letters (up, down, left, or right). Can you find the `word`? The same letter cell cannot be reused for one word.
## Examples
