...
A patient's temperature is recorded over `n` hours, forming a "mountain array". The fever rises to a peak temperature, and then "breaks" (decreases).
    1. n >= 3
    2. `temp[0] < temp[1] < ... < temp[i-1] < temp[i]`
    3. `temp[i] > temp[i+1] > ... > temp[n-1]`

Your task is to find the index (hour) at which the patient's fever was at its highest.
