...
You are a student taking a strange exam. The exam has `n` questions, and each question `i` is worth `arr[i]` points. The passing requirement is unusual: you must achieve a final score of *exactly* `sum`. You can choose to answer any subset of the questions. Is it possible to pass the exam by getting this exact score? Each question can only be used once.
## Examples
