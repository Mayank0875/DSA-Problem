...
## Description
You are an urban planner studying the skyline of a rapidly growing city. The city street is lined with buildings of various heights arranged in a row. For architectural analysis regarding sunlight and view obstruction, you want to determine the height of the nearest building to the left of the current one that is strictly shorter. For each building in the sequence, you must find the height of the closest preceding building that is strictly smaller than the current building's height. If all buildings to the left are taller or of equal height, or if there are no buildings to the left, you must indicate that no such building exists. Your task is to output these heights for the entire row.
