...
You are searching for a box in a large archive. The archive is an m x n grid of box labels (integers). The labels are sorted with these properties:
    1. Each row is sorted in non-decreasing order.
    2. The first label of each row is greater than the last label of the previous row.

Given a target label, return 1 if a box with that label exists or 0 otherwise. 
