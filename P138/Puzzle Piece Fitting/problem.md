...
## Description
A puzzle solver fits piece $y$ to slot $x$. It fits if the shape difference (XOR) divides the slot or piece ID.

The solver defines a specific relationship between a fixed integer $x$ and a candidate integer $y$. A value $y$ is considered "**fitting**" with $x$ if:
