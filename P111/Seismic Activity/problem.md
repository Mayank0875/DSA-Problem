...
## Description
Seismographs record ground motion. To identify tremors amidst background noise, geologists average the motion intensity over time windows.

You are given a **0-indexed** array `nums` of `n` integers representing motion intensities and an integer `k`.
...
## Input Format
- The first line contains an integer `n`, representing the number of motion intensities.
