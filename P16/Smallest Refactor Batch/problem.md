...
Given an array `nums` of positive "optimization points" for `n` files, and a `target` improvement score, find the length of the shortest contiguous block of files (subarray) whose total points are greater than or equal to `target`. If no block meets the target, return 0.
