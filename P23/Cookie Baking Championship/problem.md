...
It's a cookie baking championship! Your team of `n` bakers needs to bake `t` cookies. Each baker `i` is unique, and they take `k[i]` seconds to bake a single cookie. All bakers can work at the same time. Your task is to find the minimum time your team needs to have at least `t` cookies ready for the judges.
