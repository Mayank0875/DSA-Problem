...
## Description
A DJ has a playlist of songs sorted by BPM (beats per minute). They want to add a new track with a specific BPM. To keep the energy flow consistent, they need to find the correct index to insert the new track. Find the index of the first existing track with a BPM greater than or equal to the new one.
