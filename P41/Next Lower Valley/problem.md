...
## Description
You are a hiker planning a trek across a mountain ridge represented by a sequence of elevation points. For each point on the path, you want to find the elevation of the nearest future point that is strictly lower than your current altitude. This helps in planning rest stops where the air is denser. Given the list of elevations along the path, identify the next lower altitude for each position. If the terrain only goes uphill or stays level from that point onward, indicate that no lower point exists.
