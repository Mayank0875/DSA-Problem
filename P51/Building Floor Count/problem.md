...
## Description
An urban planner lists the number of floors for buildings along a street. For each building, they want to find the 'height streak': the number of consecutive buildings to the left (including the current one) that have fewer or equal floors. Given the floor counts, calculate this streak for each building.
