...
## Description
An explorer is in a room with a floor tiled with strange glyphs. To deactivate the trap, they must walk a specific sequence of glyphs. They can only step from one tile to an adjacent one. Given the floor layout and the required sequence, determine if it's possible to walk the path safely.
