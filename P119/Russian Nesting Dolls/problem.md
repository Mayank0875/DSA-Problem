...
## Description
A collector organizes matryoshka dolls. '(' is a doll top, ')' is a doll bottom.

The structure is represented by a **balanced** parentheses string $s$, where `'('` signifies opening a doll and `')'` signifies closing a doll.

The collector wants to assemble a set that has a nesting depth of exactly $k$. They can choose to skip certain dolls of the structure, effectively using a **subsequence** of the original string. However, the chosen subsequence must still form a valid, balanced sequence.
