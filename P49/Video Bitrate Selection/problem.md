...
## Description
A streaming server stores multiple versions of a video file with different bitrates, sorted low to high. A user's connection speed is detected. The server needs to select the highest quality stream that doesn't exceed the user's speed, which logically involves finding the first stream that exceeds the speed and picking the one before it, or finding the insertion point.
