...
You are in a wine cellar where bottles are stored in an m x n rack. The bottles are sorted by vintage year with the following properties:
    1. Each row is sorted by year.
    2. The first vintage in a new row is more recent (greater) than the last vintage in the previous row.

Given a target vintage year, return 1 if a bottle from that year exists or 0 otherwise. 
