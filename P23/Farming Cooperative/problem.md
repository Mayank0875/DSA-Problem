...
A farming cooperative with `n` farmers needs to harvest a total of `t` bushels of wheat. Each farmer `i` works at their own pace, taking `k[i]` seconds to harvest one bushel. If all farmers work at the same time, what is the minimum time needed for them to collectively harvest at least `t` bushels?
