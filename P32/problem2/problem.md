...
You are given an m x n matrix representing a warehouse inventory. Each cell contains a unique part ID. This inventory system has two properties:
    1. Each row (aisle) is sorted by part ID from left to right.
    2. The first part ID in any aisle is greater than the last part ID in the previous aisle.

Given a target part ID, return 1 if the part is in the warehouse or 0 otherwise. 
