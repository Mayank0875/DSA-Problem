...
A factory has a bin of $N$ parts, with costs $candidates_i$. The bin may contain many duplicate parts (e.g., ten 1-cent screws). You need to assemble a machine with a total part cost of exactly $T$. Your task is to find the *total count* of unique combinations of parts from the bin that sum to $T$. Each individual part may be used *only once*.
