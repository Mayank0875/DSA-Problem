...
You are studying the flow rates of a river measured at consecutive checkpoints along a parallel stream network. For each checkpoint, you want to find the flow rate recorded at the nearest previous checkpoint where the flow was strictly lower than the current checkpoint's flow. If no such previous checkpoint exists (either because it's the first checkpoint or all earlier flow rates were higher or equal), you should indicate this. This analysis helps understand recent reductions in water volume. Your task is to compute this value for every checkpoint given the sequence of flow rates.
