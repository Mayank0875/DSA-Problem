...
You are forming an elite squad for a special operation. You have `n` soldiers available, and each has a "skill rating". The mission requires a team with a combined skill rating of exactly `sum`. You must determine if you can form a team (a subset of your soldiers) that precisely matches this `sum`. Each soldier can only be chosen once for the team.
## Examples
