...
Given an array `nums` of positive integers representing calories found each day, and a positive `target` calorie count, find the length of the shortest contiguous number of days (subarray) to gather at least `target` calories. If it's impossible, return 0.
