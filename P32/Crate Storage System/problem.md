...
You are given an m x n matrix representing a crate storage system. Each crate has a serial number. The system is sorted with these properties:
    1. Each row is sorted by serial number.
    2. The first serial number in each row is greater than the last serial number in the previous row.

Given a target serial number, return 1 if it is in the system or 0 otherwise. 
