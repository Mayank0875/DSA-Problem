...
You are given the height profile of a simple roof gable. The height `n` values form a "mountain array": they increase from the edge to the central ridge, and then decrease.
    1. n >= 3
    2. `h[0] < h[1] < ... < h[i-1] < h[i]`
    3. `h[i] > h[i+1] > ... > h[n-1]`

Find the index of the roof's ridge (the peak).
