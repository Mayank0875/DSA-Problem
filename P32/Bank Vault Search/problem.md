...
A bank vault contains safe deposit boxes in an m x n grid. The boxes are numbered with these properties:
    1. Each row is sorted by box number.
    2. The first box number in each row is greater than the last box number in the previous row.

Given a target box number, return 1 if it is in the vault or 0 otherwise. 
