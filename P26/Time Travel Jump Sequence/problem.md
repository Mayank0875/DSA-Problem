...
A time traveler has `n` available "jump crystals," each allowing a one-way trip of a specific number of years into the future. The traveler needs to reach a year that is *exactly* `sum` years from now. Can they select a subset of their jump crystals to land on the precise target year? Each crystal can only be used once.
## Examples
