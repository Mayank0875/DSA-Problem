...
A project manager is assigning tasks for a sprint. There are `n` available tasks, each with an estimated time in hours. The manager has *exactly* `sum` hours allocated for this sprint. They need to know if it's possible to select a subset of these tasks whose total time precisely equals `sum`. Each task can only be selected once.
## Examples
