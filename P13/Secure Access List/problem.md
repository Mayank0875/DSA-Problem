...
A high-security building maintains a sorted list of all employee access codes. When an employee tries to enter, their `target` code is checked against this list. Your task is to build the core verification function. It must take the sorted list of valid codes and the employee's `target` code, and efficiently determine if access should be granted. You must write an algorithm with O(log n) runtime complexity.
