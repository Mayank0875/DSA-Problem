...
## Description
A Geiger counter records clicks per second. A smoothed average provides a stable radiation level reading.

You are given a **0-indexed** array `nums` of `n` integers representing counts per second and an integer `k`.
...
## Input Format
- The first line contains an integer `n`, representing the number of counts per second.
