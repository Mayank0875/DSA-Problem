...
## Description
A satellite receives signal strength measurements sequentially. To calibrate the receiver, engineers need to know the signal strength of the nearest future transmission that is strictly weaker than the current one. This helps in establishing noise floors. Given the list of signal strengths, find the value of the next strictly lower signal for each measurement. If all future signals are stronger or equal, output -1.
