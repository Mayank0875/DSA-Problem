...
## Description
You are a facilities manager at a university with a long corridor of lecture halls. You have a list of the seating capacities of these halls in order. For scheduling purposes, you want to find the capacity of the nearest hall to the left (previous) that has strictly fewer seats than the current hall. This helps in finding alternative, smaller venues nearby. For each hall's capacity, you need to find the value of the closest preceding hall that is strictly smaller. If all halls to the left are larger or equal, you should report this. Your task is to compute these values for all lecture halls.
