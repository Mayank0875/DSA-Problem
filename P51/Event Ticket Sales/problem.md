...
## Description
An event organizer tracks ticket sales for a series of concerts. For each concert, they calculate a 'sales streak', defined as the number of consecutive previous concerts (including the current one) where the ticket sales were less than or equal to the current concert's sales. Given the sales figures, calculate this streak.
