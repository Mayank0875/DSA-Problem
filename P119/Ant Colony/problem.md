...
## Description
An entomologist maps a colony. '{' is a chamber, '}' is a tunnel.

The structure is represented by a **balanced** parentheses string $s$, where `'{'` signifies chamber entry and `'}'` signifies chamber exit.

The mapper wants to reach the queen that has a nesting depth of exactly $k$. They can choose to skip certain chambers of the structure, effectively using a **subsequence** of the original string. However, the chosen subsequence must still form a valid, balanced sequence.
