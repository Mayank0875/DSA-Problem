...
## Description
A network router maintains a sorted list of blocked IP address ranges (represented as integers). An incoming packet has a specific IP. To optimize the search for firewall rules, the router finds the index of the first blocked IP that is greater than or equal to the packet's IP.
