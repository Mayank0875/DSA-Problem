...
You have a toy box containing $N$ toys, with weights $candidates_i$. The box may contain duplicate toys (e.g., three toy soldiers with weight 1). You want to pick out a set of toys with a total weight of exactly $T$. Your task is to find the *total count* of unique combinations of toys that sum to $T$. Each individual toy from the box may be used *only once*.
