...
## Description
An optician's order system logs lens power values. For bifocal glasses, the same lens power ID might be requested exactly twice (for left and right eyes) if the prescription is symmetrical. The inventory manager wants to find which lens powers were ordered as exact pairs. List these values.
