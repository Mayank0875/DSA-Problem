...
## Description
A device management server logs device IDs upon boot. Dual-boot systems (Windows/Linux) might log the same hardware ID twice during a maintenance cycle (once per OS). The admin wants to identify these dual-boot machines. Return the IDs logged exactly twice.
