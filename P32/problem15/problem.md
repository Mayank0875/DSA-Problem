...
A spellbook's table of contents is an m x n matrix of spell IDs. The IDs are sorted according to these rules:
    1. Each row is sorted by spell ID.
    2. The first spell ID on a new page (row) is greater than the last spell ID on the previous page.

Given a target spell ID, return 1 if the spell is in the book or 0 otherwise. 
