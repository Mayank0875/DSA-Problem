...
## Description
You are designing a tower defense game and analyzing the attack ranges of towers placed along a path. You have a sequence of range values for the towers. To balance the game, you want to find the range of the nearest previous tower that has a strictly shorter attack range than the current tower. This helps in ensuring a mix of long-range and short-range defenses. For each tower's range, you need to determine the value of the closest preceding tower that is strictly smaller. If all previous towers have larger or equal ranges, you should report this. Your task is to calculate this for the sequence of towers.
