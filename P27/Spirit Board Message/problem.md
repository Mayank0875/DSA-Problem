...
A spirit board is arranged as a `board` (grid) of letters. You are trying to see if a supernatural entity is spelling out a specific `word`. The entity can move its planchette to adjacent cells (up, down, left, or right). It will not use the same letter square twice for one word. Check if the `word` can be spelled out.
## Examples
