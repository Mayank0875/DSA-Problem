...
A space station's docking bays are numbered and sorted. When a new ship with ID `target` requests to dock, it must be assigned to the correct position in the docking queue.

Your task is to find the index for the new ship.
The index should be the first bay in the sorted list with an ID greater than or equal to the `target` ID.
If all bay IDs are smaller, the ship is added to the end of the queue.
