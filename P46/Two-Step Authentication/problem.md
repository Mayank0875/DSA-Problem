...
## Description
A security system logs user IDs when they pass an authentication step. A complete login requires exactly two steps (password and bio-metric). The security officer wants to identify user IDs that have successfully completed exactly two steps today, filtering out incomplete (1) or retry-heavy (>2) attempts.
