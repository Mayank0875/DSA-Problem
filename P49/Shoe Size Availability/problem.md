...
## Description
A shoe store inventory system lists available sizes for a sneaker, sorted from smallest to largest. A customer asks for a specific size. If that exact size isn't available, the salesperson looks for the smallest size that is larger than requested. Find the index of the first size that is greater than or equal to the customer's request.
