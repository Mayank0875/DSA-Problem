...
## Description
Compression algorithm analyzes frames. '<' is a keyframe, '>' is a delta.

The structure is represented by a **balanced** parentheses string $s$, where `'<'` signifies encoding and `'>'` signifies decoding.

The codec wants to compress data that has a nesting depth of exactly $k$. They can choose to skip certain frames of the structure, effectively using a **subsequence** of the original string. However, the chosen subsequence must still form a valid, balanced sequence.
