...
## Description
You are developing a health monitoring application that tracks a user's heart rate over time during a workout session. For each heart rate reading, the user wants to know the value of the nearest previous reading that was strictly lower. This feature helps users understand their recovery phases and intensity spikes relative to recent history. For every heart rate value recorded in the sequence, you must find the most recent prior reading that was strictly less than the current value. If no such reading exists (because the user is warming up or maintaining a high rate), you must indicate this. Your task is to compute this comparison for the entire sequence of heart rate data.
