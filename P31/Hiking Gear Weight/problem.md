...
A hiker has $N$ items in their backpack, with weights $candidates_i$. The pack may contain duplicates (e.g., two water bottles of weight 2). The hiker wants to select a subset of items with a total weight of exactly $T$. Your task is to find the *total count* of unique combinations of items that sum to $T$. Each item may be used *only once*.
