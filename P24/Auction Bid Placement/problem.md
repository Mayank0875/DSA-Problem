...
You are building an auction system. All current bids are stored in a sorted list, from lowest to highest. A new bid of `target` amount is placed.

Your task is to find the index where this new bid should be inserted to keep the list sorted.
The index should be the first one in the list with a bid amount greater than or equal to `target`.
If all other bids are lower, the new bid is placed at the end.
