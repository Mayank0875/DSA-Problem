...
You are searching a massive log file that has been organized into an m x n matrix of timestamps (integers). The log file guarantees these properties:
    1. Each row is sorted by timestamp.
    2. The first timestamp of each row is greater than the last timestamp of the previous row.

Given a target timestamp, return 1 if it exists in the log or 0 otherwise. 
