...
## Description
You are working as a meteorologist analyzing historical weather patterns for a specific region. You have a dataset consisting of daily maximum temperatures recorded sequentially over time. To understand cooling trends and fluctuations, you need to identify the nearest previous day that was strictly colder than the current day. For every recorded temperature, your goal is to find the temperature value of the most recent preceding day where the temperature was strictly lower. If no such day exists (either because it is the start of the record or all previous days were warmer or equal), you should report -1. Your task is to process the daily temperature list and compute this value for each day to help visualize the temperature fluctuations.
