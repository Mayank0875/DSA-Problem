...
You are a data analyst tracking a product's weekly sales. The sales data forms a "peak array", meaning the sales increase, reach a maximum, and then decrease. An array `arr` of size `n` has:
    1. n >= 3
    2. An index `i` (0 < i < n - 1) where:
        `arr[0] < arr[1] < ... < arr[i-1] < arr[i]`
        `arr[i] > arr[i+1] > ... > arr[n-1]`

Your task is to find the index of the week with peak sales.
