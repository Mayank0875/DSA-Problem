...
A factory's quality control scanner has processed N items on a conveyor belt. '1' represents a perfect item, and '0' represents a defective item. A production batch is considered "acceptable" if any contiguous segment of items contains no more than `k` defective items. You need to find the length of the longest acceptable contiguous batch.
