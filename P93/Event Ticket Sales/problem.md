...
## Description
Tickets up to `T` are sold. Backstage passes are hidden in tickets where the digit sum is divisible by `B`.

To find passes, you must calculate the count of tickets between $1$ and $T$ (inclusive) that satisfy a specific condition: the sum of their digits (in base 10) must be perfectly divisible by $B$.
