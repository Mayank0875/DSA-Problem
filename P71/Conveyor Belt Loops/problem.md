...
## Description
Items circulate on closed conveyor loops.

There are n items.
Loops hold at least 2 items.
Each item is behind exactly one other item.
...
5 <-> 6.
Maximum:
These could be 3 separate loops of 2 items each.
Minimum:
Since we only know one neighbor per item, these pairs could actually be segments of a bigger ring. All items can be arranged into one single ring:
1 - 2 - 3 - 4 - 5 - 6 - 1.
...
- The second line contains n space-separated integers a1, a2, ..., an, where ai is the neighbor remembered by the i-th item.
