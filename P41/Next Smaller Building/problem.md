...
## Description
You are an architect reviewing a street layout plan where buildings are arranged in a line from left to right. For each building, you need to identify the height of the nearest building to its right that is strictly shorter. This is crucial for analyzing shadow casting and wind tunnel effects. Given the heights of the buildings in order, find the height of the next smaller building for each one. If no shorter building exists to the right, indicate this.
