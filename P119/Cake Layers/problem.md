...
## Description
A baker stacks cakes. '(' is a sponge layer, ')' is icing.

The structure is represented by a **balanced** parentheses string $s$, where `'('` signifies sponge and `')'` signifies icing.

The baker wants to make a wedding cake that has a nesting depth of exactly $k$. They can choose to skip certain tiers of the structure, effectively using a **subsequence** of the original string. However, the chosen subsequence must still form a valid, balanced sequence.
