...
An assembly line has `n` stations. Each station adds a positive quality score (in array `nums`) to a product. The product must achieve a `target` quality score. Find the length of the shortest contiguous run of stations (subarray) that a product can pass through to get a score greater than or equal to `target`. If no run meets the target, return 0.
