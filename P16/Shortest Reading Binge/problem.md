...
Given an array `nums` where each element is the number of pages in a book chapter, and a positive integer `target` (your reading goal), find the shortest contiguous sequence of chapters (subarray) whose total page count is greater than or equal to the `target`. If no such sequence exists, return 0.
