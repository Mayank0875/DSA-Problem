...
## Description
A mining pool tracks hash rates. To estimate payout, the pool calculates the average hash rate over a sliding window of time.

You are given a **0-indexed** array `nums` of `n` integers representing hash rates and an integer `k`.
...
## Input Format
- The first line contains an integer `n`, representing the number of hash rates.
