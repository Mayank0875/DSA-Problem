...
## Description
Genes regulate each other in feedback loops.

There are n genes.
Loops involve at least 2 genes.
We identified one regulator for each gene.
...
5 <-> 6.
Maximum:
These could be 3 separate loops of 2 genes each.
Minimum:
Since we only know one neighbor per gene, these pairs could actually be segments of a bigger ring. All genes can be arranged into one single ring:
1 - 2 - 3 - 4 - 5 - 6 - 1.
...
- The second line contains n space-separated integers a1, a2, ..., an, where ai is the neighbor remembered by the i-th gene.
