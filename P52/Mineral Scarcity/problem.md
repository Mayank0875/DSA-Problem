...
## Description
A geologist has collected various mineral samples, each identified by an ID. To assist in analysis, the minerals need to be arranged by scarcity. Samples that are found less often (lower count) should be processed first. If multiple mineral types have the same count, the one with the higher ID value takes precedence.
