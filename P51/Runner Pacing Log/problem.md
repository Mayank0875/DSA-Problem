...
## Description
A marathon runner logs their speed for every kilometer. They want to know their 'consistency streak' for each kilometer. This is defined as the number of consecutive previous kilometers (including the current one) where their speed was less than or equal to the current kilometer's speed. Given the list of recorded speeds, calculate the streak for each kilometer.
