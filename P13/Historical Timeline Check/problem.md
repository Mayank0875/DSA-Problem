...
A historian is analyzing a sorted timeline of significant event years. They want to know if any event is recorded for a specific `target` year. Your function will receive the sorted list of event years and the `target` year. You must efficiently determine if the target year is present in the timeline. You must write an algorithm with O(log n) runtime complexity.
