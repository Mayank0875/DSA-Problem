...
A journey is broken into `n` legs, with positive distances in `nums`. Given a `target` distance, find the length (in number of legs) of the shortest contiguous sequence of legs (subarray) whose total distance is greater than or equal to `target`. If no such sequence exists, return 0.
