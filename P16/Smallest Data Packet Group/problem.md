...
You are receiving a stream of data packets, with their positive sizes in an array `nums`. You need to buffer at least `target` bytes. Find the length of the shortest contiguous sequence of packets (subarray) whose total size is greater than or equal to the `target`. If no such sequence exists, return 0.
