...
## Description
A browser engine parses DOM elements. '<div>' opens, '</div>' closes.

The structure is represented by a **balanced** parentheses string $s$, where `'<'` signifies opening a tag and `'>'` signifies closing a tag.

The parser wants to render a layout that has a nesting depth of exactly $k$. They can choose to skip certain tags of the structure, effectively using a **subsequence** of the original string. However, the chosen subsequence must still form a valid, balanced sequence.
